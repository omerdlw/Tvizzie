...
export function EmptyState({ action, description, icon, title, className }) {
  return (
    <div
      className={cn(
        'group flex flex-col items-center justify-center py-12 text-center',
        className
      )}
    >
      <div className="relative mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-white/10 bg-white/5 transition-transform duration-500 group-hover:scale-110">
        <Icon
          icon={icon}
          size={28}
          className="text-white/40 transition-colors duration-300 group-hover:text-white/60"
        />
      </div>
      <div className="flex max-w-[280px] flex-col items-center">
        ...
        {description && (
          <p className="mt-1.5 text-xs leading-relaxed font-medium text-white/40">
            {description}
          </p>
        )}
      </div>
...
export function FullScreenEmptyState({ action, description, icon, title }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      {description && (
        <p className="mt-2 max-w-sm text-sm leading-relaxed font-medium text-white/30 sm:text-base">
          {description}
        </p>
      )}
      {action && (
