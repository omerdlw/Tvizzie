...
} from '@/components/profile/profile-empty-state'
import { ProfileHero } from '@/components/profile/profile-hero'
import { ListCard } from '@/components/profile/profile-list-card'
import { MediaGrid } from '@/components/profile/profile-media-grid'
...
export default function ProfilePage({
  ...
  const activeTab = useMemo(() => {
    ...
    const requestedTab = searchParams.get('tab') || 'favorites'
    return PROFILE_TABS.includes(requestedTab) ? requestedTab : 'favorites'
  ...
  function handleTabChange(tab) {
    ...
    const target = tab === 'favorites' ? base : `${base}/tab/${tab}`
      ...
      <div className="w-screen h-screen center p-4">
        <EmptyState
          ...
          action={
            <Link
              href="/"
              className="rounded-full bg-white px-5 py-3 text-xs font-semibold tracking-[0.18em] text-black uppercase transition hover:bg-white/85"
            >
              Back Home
            </Link>
          }
            ...
            action={
              isOwner ? (
                <button
                  ...
                  className="flex cursor-pointer items-center gap-2 rounded-full bg-white px-5 py-3 text-xs font-semibold tracking-[0.2em] text-black uppercase transition hover:bg-white/90"
                >
