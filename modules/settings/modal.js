    ...
    return (
      <Switch
        ...
        className={{
          ...
          track:
            'h-6 w-11   p-0.5 transition-colors duration-200',
          trackActive: '',
  ...
  if (control === 'select') {
    return (
      <select
        ...
        className="w-full  border border-white/5  px-4 py-3 text-sm text-white transition-colors outline-none placeholder:text-white focus:border-white/10 focus: font-medium"
      >
...
export default function SettingsModal({ close, header }) {
  ...
  return (
    <Container
      ...
      close={close}
    >
      <div className="flex w-full flex-col gap-3 p-4 text-sm text-white">
        <div className="flex items-center justify-between gap-2  border border-white/5  px-4 py-3">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold tracking-widest text-white uppercase">
              Storage
            </span>
            <span className="font-medium text-white">{storageKey}</span>
          </div>
          {hasDefinitions ? (
            <button
              type="button"
        ...
        {!isHydrated ? (
          <div className=" border border-white/5  px-4 py-3 text-sm text-white">
            Loading settings
          </div>
        ) : null}
        ...
        {isHydrated && !hasDefinitions ? (
          <div className=" border border-white/5  px-4 py-3 text-sm text-white">
            No setting definitions are registered yet. The module is ready and
            persists decisions centrally under <strong>{storageKey}</strong>.
            Register definitions through the Settings API to render controls
            dynamically in this modal
          </div>
        ) : null}

        {isHydrated && hasDefinitions
          ? Object.entries(definitionGroups).map(
              ([groupKey, groupDefinitions]) => (
                <section key={groupKey} className="flex flex-col gap-2">
                  <div className="px-1 text-[10px] font-bold tracking-widest text-white uppercase">
                    {groupKey}
                  </div>

                  {groupDefinitions.map((definition) => {
                    const currentValue = getSetting(
                      definition.path,
                    ...
                    return (
                      <div
                        key={definition.path}
                        className="flex flex-col gap-2  border border-white/5  p-4"
                      >
                        {control === 'switch' ? (
                          renderControl({
                            definition,
                            value: currentValue,
                            onChange: (nextValue) =>
                              setSetting(definition.path, nextValue),
                          })
                        ) : (
                          <>
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-sm font-semibold text-white">
                                  {definition.label ||
                                    formatLabel(definition.path)}
                                </span>
                                <button
                                  type="button"
                                  className="text-[10px] font-bold tracking-widest text-white uppercase transition hover:text-white"
                        ...
                        <div className="flex flex-wrap gap-2">
                          {definition.storage.map((target) => (
                            <span
                              key={target}
                              className=" border border-white/5 px-2.5 py-1 text-[10px] font-bold tracking-widest text-white uppercase"
                            >
                              {target}
                            </span>
                          ))}
