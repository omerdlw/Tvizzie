...
function toSourceRecord(rawRecord, source = DEFAULT_SOURCE) {
  ...
  if (isObject(rawRecord) && hasOwnProperty(rawRecord, 'value')) {
    ...
    return {
      ...
      source:
        typeof rawRecord.source === 'string' ? rawRecord.source : source,
...
function applyOperation(state, operation) {
  ...
  if (operation.kind === 'unregister') {
    return removeSourceRecord(state, operation.type, operation.key, operation.source)
  }
...
export function RegistryProvider({ children, enableHistory = true }) {
  ...
  const getHistory = useCallback((limit = HISTORY_LIMIT) => {
    if (!historyEnabled) return []
...
function useModalRegistryActions() {
  ...
  return useMemo(
    () => ({ batch: modalBatch, register: modalRegister, unregister: modalUnregister }),
...
export function useNavRegistryActions() {
  ...
  return useMemo(
    () => ({ batch: navBatch, register: navRegister, unregister: navUnregister }),
