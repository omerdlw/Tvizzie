...
export function RegistryDebugPanel({ limit = 120 }) {
  ...
  return (
    <div className="pointer-events-none fixed right-4 bottom-4 z-[1500] flex w-[360px] max-w-[calc(100vw-2rem)] flex-col gap-2">
      <button
        className="pointer-events-auto rounded-md border border-white/20 bg-black/75 px-3 py-2 text-left text-xs font-medium text-white"
      ...
      {isOpen && (
        <div className="pointer-events-auto overflow-hidden rounded-md border border-white/20 bg-black/85 text-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-white/10 px-3 py-2">
            <span className="text-[11px] font-semibold tracking-wide">REGISTRY</span>
            <div className="flex items-center gap-1.5">
              <button
                className="rounded border border-white/20 px-2 py-1 text-[10px] font-medium text-white/90"
                onClick={handleCopy}
                type="button"
              >
                {copyLabel}
          ...
          <div className="pointer-events-auto max-h-[70vh] overflow-auto overscroll-contain px-2 py-2">
            {items.length === 0 ? (
              <div className="rounded bg-white/5 px-2 py-3 text-center text-xs text-white/60">
              ...
              items.map((entry, index) => {
                ...
                return (
                  <div
                    ...
                    className="mb-2 rounded bg-white/5 px-2 py-2 select-text"
                  >
                            ...
                            {entry.operations.map((operation, operationIndex) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="mb-1 rounded bg-white/5 px-1.5 py-1"
                                >
                                  ...
                                  {operationPayloadText && (
                                    <div className="mt-1 text-[10px] leading-4 text-white/60 break-words">
                                      {operationPayloadText}
                                    </div>
                                  )}
                                </div>
