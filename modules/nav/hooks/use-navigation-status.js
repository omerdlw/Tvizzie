...
import { SEMANTIC_SURFACE_CLASSES } from '@/lib/constants'
...
const ERROR_STATUS_TYPES = new Set([
  'ACCOUNT_DELETE',
  'APP_ERROR',
  'API_ERROR',
  'NOT_FOUND',
...
const AUTH_STATUS_STORAGE_KEY = 'tvz_nav_auth_status'
...
function ErrorActions({ onRetry, onRefresh }) {
  return (
    <div className="mt-2.5 flex items-center gap-2">
      <Button
        className="error-classes center w-full cursor-pointer px-4 py-2 text-sm font-semibold rounded-[12px] transition-colors"
        onClick={(event) => {
...
function createProgressIcon() {
  return <Spinner size={18} className="text-white" />
}
...
export function useNavigationStatus() {
  ...
  useEffect(() => {
    ...
    const unsubscribeSignUp = globalEvents.subscribe(
      EVENT_TYPES.AUTH_SIGN_UP,
      (eventData) => {
        const user = eventData?.session?.user
      ...
      (eventData) => {
        ...
        updateStatus(
          createOverlayStatus({
            ...
            style:
              eventData?.style ||
              getStatusTheme(eventData?.themeType || 'LOGIN'),
    ...
    const unsubscribeNotFound = globalEvents.subscribe(
      EVENT_TYPES.NAV_NOT_FOUND,
      (eventData) => {
        if (eventData?.clear) {
          setStatus((currentStatus) =>
