...
// Flow functions own orchestration only. Storage, transport and React state
// stay behind the callbacks supplied by the runtime seam.
...
export async function runAuthRefreshSession({
  ...
  try {
    // Adapters may expose only getSession; do not call a missing refresh method
    // when a current session exists.
    const nextSession =
      activeSession && adapter?.refreshSession
