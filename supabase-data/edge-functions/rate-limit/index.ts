...
import { clamp, normalizeLower, normalizeTrim, parseBoolean, parseNumber } from '../_shared/normalize.ts';
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    for (const dimension of input.dimensions) {
      ...
      const key = [keyPrefix, input.namespace, dimension.id, valueHash, String(windowBucket)].join(':');
    ...
    return errorResponse(status, String((error as Error)?.message || 'Rate limit function failed'));
  }
});
