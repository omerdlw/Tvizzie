...
async function getMediaSocialProofResource(
  ...
  const followingIds = followingRows.map((row) => normalizeTrim(row.following_id)).filter(Boolean);
  ...
  const [likesResult, watchlistResult, reviewsResult] = await Promise.all([
    admin.from('likes').select('user_id').eq('media_key', mediaKey).in('user_id', followingIds),
    admin.from('watchlist').select('user_id').eq('media_key', mediaKey).in('user_id', followingIds),
    admin.from('media_reviews').select('user_id').eq('media_key', mediaKey).in('user_id', followingIds),
  ]);
...
async function getAccountSocialProofResource(
  ...
  const viewerFollowersResult = await withQueryTimeout(
    admin.from('follows').select('follower_id').eq('following_id', viewerId).eq('status', 'accepted'),
    {
  ...
  const viewerLikesResult = await withQueryTimeout(
    admin.from('likes').select('media_key,title').eq('user_id', viewerId),
    {
      fallbackValue: { data: [], error: null },
      timeoutMs: 4000,
    }
