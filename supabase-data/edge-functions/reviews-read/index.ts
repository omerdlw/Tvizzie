...
async function loadListSubject(
  ...
  return {
    ...
    subjectKey: createListReviewLikeKey(normalizeTrim(listResult.data.user_id), normalizeTrim(listResult.data.id)),
...
async function getListReviews(
  ...
  return (result.data || []).map((row) =>
    normalizeReviewRow(
      row as Record<string, unknown>,
      subject,
      likesMap.get(`${normalizeTrim(subject.subjectKey)}:${normalizeTrim((row as Record<string, unknown>).user_id)}`) ||
        []
    )
  );
}
...
Deno.serve(async (request: Request) => {
  try {
    ...
    return errorResponse(mapErrorToStatus(error), String((error as Error)?.message || 'reviews-read failed'));
  }
});
