...
async function enforceSignupRateLimit(event: BeforeUserCreatedEvent): Promise<void> {
  ...
  const keyPrefix =
    normalizeLower(Deno.env.get("SIGNUP_RATE_LIMIT_KEY_PREFIX")).replace(/[^a-z0-9:_-]/g, "") ||
    "tvz:signup:hook:v1"
...
function enforceSignupPolicies(event: BeforeUserCreatedEvent): void {
  ...
  if (allowedDomains.size > 0 && (!domain || !allowedDomains.has(domain))) {
    throw new SignupPolicyError(
      "Please sign up with an approved email domain.",
      403
    )
  }
}
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const verification = await readAndVerifyWebhook<BeforeUserCreatedEvent>(
      request,
    ...
    return runtimeErrorResponse(
      String((error as Error)?.message || "Signup validation is temporarily unavailable")
    )
  }
})
