...
import { buildAvailableUsername, getDefaultUsernameBase, normalizeOptionalUrl, normalizeProfileResponse, validateUsername } from "../_shared/account.js"
...
async function claimUsernameManual(
  ...
  if (
    failIfProfileHasUsername &&
    normalizeValue(currentProfile?.username)
  ) {
...
async function runEnsureAction(
  admin: ReturnType<typeof createAdminClient>,
...
async function runUpdateAction(
  ...
  if (shouldPromoteFollowers) {
    await admin
      .rpc(PROMOTE_PENDING_RPC, { p_user_id: userId })
      .catch(() => null)
  }
...
async function runSyncEmailAction(
  admin: ReturnType<typeof createAdminClient>,
  payload: ProfileWriteRequest
) {
  const userId = normalizeValue(payload.userId)
