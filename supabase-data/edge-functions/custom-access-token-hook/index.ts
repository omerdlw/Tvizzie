...
function buildClaims(inputClaims: Record<string, unknown>): Record<string, unknown> {
  const includeUserMetadata = parseBoolean(
    Deno.env.get("CUSTOM_TOKEN_INCLUDE_USER_METADATA"),
    false
  )
  const includeAppMetadata = parseBoolean(
    Deno.env.get("CUSTOM_TOKEN_INCLUDE_APP_METADATA"),
    true
  )
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const verification = await readAndVerifyWebhook<AccessTokenHookPayload>(
      request,
    ...
    return runtimeErrorResponse(
      String((error as Error)?.message || "Custom access token hook failed")
    )
  }
})
