...
function normalizeActivityRow(row: ActivityRow) {
  ...
  const nestedPayload =
    payload.payload && typeof payload.payload === 'object' ? (payload.payload as Record<string, unknown>) : payload;
...
function paginateItems(items: Record<string, unknown>[], cursor: unknown, pageSize: unknown) {
...
async function enrichActivityItems(admin: ReturnType<typeof createAdminClient>, items: Record<string, unknown>[]) {
  if (!items.length) {
    return [];
  }
      ...
      listStateMap.set(key, {
        ...
        previewItems: Array.isArray(payload.previewItems)
          ? payload.previewItems
              .map((item) => normalizeListPreviewItem(item))
              .filter(Boolean)
              .slice(0, 3)
          : [],
      });
...
async function fetchAcceptedFollowingIds(admin: ReturnType<typeof createAdminClient>, userId: string) {
  const result = await admin
    .from('follows')
    .select('following_id')
  ...
  return (result.data || []).map((row) => normalizeValue(row.following_id)).filter(Boolean);
}
...
Deno.serve(async (request: Request) => {
  try {
    ...
    const followingIds = scope === 'following' ? await fetchAcceptedFollowingIds(admin, userId).catch(() => []) : [];
