...
type NotificationsControlAction =
  | "list"
  | "unread-count"
  | "mark-read"
  | "mark-all-read"
...
function normalizeAction(value: unknown): NotificationsControlAction {
  ...
  throw new Error(
    "action must be one of: list, unread-count, mark-read, mark-all-read, delete"
  )
}
...
function normalizeNotificationRow(row: Record<string, unknown>) {
  const metadata = row.metadata && typeof row.metadata === "object"
    ...
    : {}
  const actor = metadata.actor && typeof metadata.actor === "object"
...
function hasSupportedNotificationPayload(
  ...
  const subject = payload.subject && typeof payload.subject === "object"
    ...
    : null
  const list = payload.list && typeof payload.list === "object"
...
Deno.serve(async (request: Request) => {
  try {
    ...
    return errorResponse(
      ...
      String((error as Error)?.message || "notifications-control failed")
    )
  }
})
