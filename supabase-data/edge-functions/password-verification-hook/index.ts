...
async function exceededFailureThreshold(event: PasswordVerificationHookEvent): Promise<boolean> {
  const maxAttempts = Math.max(
    0,
    parseNumber(Deno.env.get("PASSWORD_FAIL_MAX_ATTEMPTS"), 10)
  )
  ...
  const windowMs = Math.max(
    1_000,
    parseNumber(Deno.env.get("PASSWORD_FAIL_WINDOW_MS"), 900_000)
  )
  const keyPrefix =
    normalizeLower(Deno.env.get("PASSWORD_FAIL_KEY_PREFIX")).replace(/[^a-z0-9:_-]/g, "") ||
    "tvz:password:hook:v1"
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const verification = await readAndVerifyWebhook<PasswordVerificationHookEvent>(
      request,
      HOOK_SECRET_ENV_NAMES
    )
    ...
    if (!verification.verified || !verification.event) {
      ...
      return runtimeErrorResponse(
        verification.error?.message || "Webhook signature verification failed"
      )
    }
    ...
    const shouldLogoutUser = parseBoolean(
      Deno.env.get("PASSWORD_REJECT_SHOULD_LOGOUT_USER"),
      false
    )
    ...
    return runtimeErrorResponse(
      String((error as Error)?.message || "Password verification hook failed")
    )
  }
})
