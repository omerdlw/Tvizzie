...
async function loadListSubjectMap(
  ...
  listRows.forEach((row) => {
    ...
    const ownerUsername =
      normalizeTrim(ownerSnapshot.username) ||
      ownerMap.get(userId) ||
      userId
    ...
    listMap.set(listId, {
      ...
      subjectPreviewItems: Array.isArray(payload.previewItems)
...
async function fetchAuthoredReviews(
  ...
  const listMap = await loadListSubjectMap(
    admin,
    listRows.map((row) => normalizeTrim(row.list_id)).filter(Boolean)
  )
  ...
  const mediaReviews = mediaRows.map((row) =>
    normalizeReviewRow(
      row,
      {},
      likesMap.get(`${normalizeTrim(row.media_key)}:${normalizeTrim(row.user_id)}`) || []
    )
  )
...
async function fetchLikedReviews(
  ...
  for (let index = 0; index < mediaKeys.length; index += 100) {
    ...
    ;(reviewResult.data || []).forEach((row) => {
      ...
      mediaReviewMap.set(
        `${normalizeTrim(normalized.media_key)}:${normalizeTrim(normalized.user_id)}`,
        normalized
      )
    })
  }
  ...
  for (let index = 0; index < listIds.length; index += 100) {
    ...
    ;(reviewResult.data || []).forEach((row) => {
      const normalized = row as Record<string, unknown>
      listReviewMap.set(
        `${normalizeTrim(normalized.list_id)}:${normalizeTrim(normalized.user_id)}`,
        normalized
      )
    })
  }
  ...
  const mediaReviews = mediaLikeRows
    .map((likeRow) => {
      const review = mediaReviewMap.get(
        `${normalizeTrim(likeRow.media_key)}:${normalizeTrim(likeRow.review_user_id)}`
      )
...
Deno.serve(async (request: Request) => {
  try {
    ...
    return errorResponse(
      ...
      String((error as Error)?.message || "account-reviews-feed failed")
    )
  }
})
