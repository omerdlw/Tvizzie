...
async function exceededFailureThreshold(event: MfaVerificationHookEvent): Promise<boolean> {
  ...
  const keyPrefix =
    normalizeLower(Deno.env.get("MFA_FAIL_KEY_PREFIX")).replace(/[^a-z0-9:_-]/g, "") ||
    "tvz:mfa:hook:v1"
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const verification = await readAndVerifyWebhook<MfaVerificationHookEvent>(
      request,
    ...
    const rejectMessage =
      String(Deno.env.get("MFA_REJECT_MESSAGE") || "").trim() ||
      "You have exceeded maximum number of MFA attempts."
    ...
    return runtimeErrorResponse(
      String((error as Error)?.message || "MFA verification hook failed")
    )
  }
})
