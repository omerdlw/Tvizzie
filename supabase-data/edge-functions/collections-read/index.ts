...
function normalizeMediaPayload(
  ...
  const entityType = normalizeMediaType(
  ...
  return {
    ...
    backdrop_path:
      normalizeTrim(payload.backdrop_path || payload.backdropPath || row.backdrop_path) ||
      null,
    ...
    mediaKey:
      normalizeTrim(payload.mediaKey || row.media_key) ||
    ...
    poster_path:
      normalizeTrim(payload.poster_path || payload.posterPath || row.poster_path) ||
      null,
...
function normalizeListOwnerSnapshot(
  value: Record<string, unknown> = {},
  fallbackOwnerId: string | null = null
) {
  const ownerId =
    normalizeTrim(value.id) ||
    normalizeTrim(fallbackOwnerId) ||
    null
  ...
  return ownerId
    ? {
        ...
        displayName:
          normalizeTrim(value.displayName || value.username) ||
          "Anonymous User",
        id: ownerId,
  ...
  try {
    ...
    if (resource === "likes") {
      ...
      const data = (result.data || [])
        .map((row) => normalizeMediaPayload((row as Record<string, unknown>).payload as Record<string, unknown> || {}, row as Record<string, unknown>))
    ...
    if (resource === "lists") {
      ...
      const likesMap = await countListLikesByListIds(
        admin,
        rows.map((row) => normalizeTrim(row.id)).filter(Boolean)
      )
    ...
    if (resource === "liked-lists") {
      ...
      const listIds = [...new Set((likesResult.data || [])
        .map((row) => normalizeTrim((row as Record<string, unknown>).list_id))
        .filter(Boolean))]
    ...
    if (resource === "like-status") {
      ...
      return jsonResponse(200, {
        ...
        resource,
        data: {
          isLiked: Boolean(result.data),
          like: result.data
            ? normalizeMediaPayload(
                ((result.data as Record<string, unknown>).payload as Record<string, unknown>) || {},
    ...
    if (resource === "watchlist-status") {
      ...
      return jsonResponse(200, {
        ...
        resource,
        data: {
          isInWatchlist: Boolean(result.data),
          item: result.data
            ? normalizeMediaPayload(
                ((result.data as Record<string, unknown>).payload as Record<string, unknown>) || {},
                result.data as Record<string, unknown>
    ...
    if (resource === "watched-status") {
      ...
      return jsonResponse(200, {
        ...
        resource,
        data: {
          isWatched: Boolean(result.data),
          watched: result.data
            ? normalizeWatchedRow(result.data as Record<string, unknown>)
            : null,
        },
      })
    }
    ...
    return errorResponse(
      ...
      String((error as Error)?.message || "collections-read failed")
    )
  }
})
