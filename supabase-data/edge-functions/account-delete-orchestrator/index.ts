...
async function purgeAccountData(admin: ReturnType<typeof createAdminClient>, userId: string) {
  ...
  steps.push(await executeDeleteStep('review_likes', () => admin.from('review_likes').delete().eq('user_id', userId)));

  steps.push(await executeDeleteStep('list_likes', () => admin.from('list_likes').delete().eq('user_id', userId)));

  steps.push(
    await executeDeleteStep('likes', () => admin.from('likes').delete().eq('user_id', userId), { optional: true })
  );

  steps.push(
    await executeDeleteStep('watchlist', () => admin.from('watchlist').delete().eq('user_id', userId), {
      optional: true,
    })
  );

  steps.push(
    await executeDeleteStep('watched', () => admin.from('watched').delete().eq('user_id', userId), { optional: true })
  );

  steps.push(
    await executeDeleteStep('media_reviews', () => admin.from('media_reviews').delete().eq('user_id', userId), {
      optional: true,
    })
  );

  steps.push(
    await executeDeleteStep('list_reviews', () => admin.from('list_reviews').delete().eq('user_id', userId), {
      optional: true,
    })
  );

  steps.push(
    await executeDeleteStep('lists', () => admin.from('lists').delete().eq('user_id', userId), { optional: true })
  );
  ...
  steps.push(
    await executeDeleteStep('activity', () => admin.from('activity').delete().eq('user_id', userId), { optional: true })
  );

  steps.push(
    await executeDeleteStep('notifications', () => admin.from('notifications').delete().eq('user_id', userId))
  );

  steps.push(await executeDeleteStep('usernames', () => admin.from('usernames').delete().eq('user_id', userId)));

  steps.push(await executeDeleteStep('profiles', () => admin.from('profiles').delete().eq('id', userId)));
