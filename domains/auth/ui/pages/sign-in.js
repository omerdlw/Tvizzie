...
import { signInWithPassword } from '@/domains/auth/client/sign-in';
import { AUTH_PURPOSE, INITIAL_RESET_FLOW } from '@/domains/auth/utils/constants';
import {
  AUTH_ROUTES,
  buildAuthHref,
...
import { ForgotPasswordAction } from '@/domains/shell/navigation/actions/forgot-password-action';
import { OAuthProviderList } from '@/domains/auth/ui/components/oauth-provider-list';
...
export default function SignInView() {
  ...
  const finalizeVerifiedSignIn = async (verificationResult) => {
    ...
    if (postAuthRedirect === '/account') {
      try {
        const payload = await requestJson('/api/account/profile', { retryCount: 0 });
      ...
      {isResetMode ? (
        ...
        <form onSubmit={handleSubmit} className={AUTH_PAGE_FORM_CLASS}>
          <div className="flex flex-col items-center text-center">
            ...
            >
              <Image src="/tvizzie.png" alt="Tvizzie" width={64} height={64} className="size-16" />
            </Link>
