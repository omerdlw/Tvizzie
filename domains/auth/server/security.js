...
import { SUPABASE_URL } from '@/infrastructure/supabase/server';
import { SUPABASE_SERVICE_ROLE_KEY } from '@/infrastructure/supabase/server';
...
export async function consumeStepUp(request, { purpose, userId, email = null }) {
  const stepUp = assertStepUp(request, { email, purpose, userId });
  const normalizedUserId = extractUuid(userId);

  if (!normalizedUserId) throw new Error('Step-up verification is invalid');

  const result = await createAdminClient().rpc('consume_auth_step_up', {
    p_challenge_key: stepUp.challengeJti,
    p_purpose: normalizeLowerValue(purpose),
    p_user_id: normalizedUserId,
  });

  if (result.error) {
    throw new Error(result.error.message || 'Step-up verification could not be consumed');
  }

  const consumed = Array.isArray(result.data) ? result.data[0] : result.data;
  if (!consumed?.challenge_key) {
    throw new Error('Step-up verification has already been used');
  }
...
const RATE_LIMIT_FALLBACK_MODES = new Set(['auto', 'memory', 'fail-closed']);

function resolveRateLimitFallbackMode() {
  const configuredMode = normalizeLowerValue(process.env.AUTH_RATE_LIMIT_FALLBACK_MODE);
  if (RATE_LIMIT_FALLBACK_MODES.has(configuredMode)) return configuredMode;

  if (process.env.NODE_ENV === 'production') return 'fail-closed';
  return RATE_LIMIT_FALLBACK_MODE === 'auto' ? 'memory' : RATE_LIMIT_FALLBACK_MODE;
}

function createRateLimitUnavailableError() {
  const error = new Error('Authentication rate limiting is temporarily unavailable');
...
export async function enforceSlidingWindowRateLimit({
  ...
  const fallbackMode = resolveRateLimitFallbackMode();
  let rateLimitServiceAvailable = false;
    ...
    if (internalToken) {
      try {
        ...
        if (response.ok && payload?.allowed === true) {
          rateLimitServiceAvailable = true;
          return;
        }
      } catch (err) {
  ...
  if (fallbackMode === 'fail-closed' || (fallbackMode === 'auto' && !rateLimitServiceAvailable)) {
...
export const AUTH_RATE_LIMIT_POLICIES = Object.freeze({
  ...
  VERIFICATION_SEND: Object.freeze({
    dimensions: Object.freeze({ device: 20, email: 10, ip: 40 }),
    dimensionMessages: Object.freeze({
  ...
  MFA_PRIMARY_VERIFY: Object.freeze({
    dimensions: Object.freeze({ device: 20, email: 12, ip: 40 }),
    dimensionMessages: Object.freeze({
      default: 'Too many authenticator attempts from this network',
      device: 'Too many authenticator attempts from this device',
      email: 'Too many authenticator attempts for this account',
    }),
    message: 'Too many authenticator attempts',
    namespace: 'auth:mfa-primary:verify',
    windowMs: 15 * 60 * 1000,
  }),
