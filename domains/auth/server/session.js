...
import { SUPABASE_FALLBACK_TIMEOUT_MS } from '@/domains/auth/utils/constants';
import { randomBytes } from 'crypto';
import { createSupabaseResponseClient } from '@/infrastructure/supabase/server';
import {
...
const VERIFIED_TOKEN_INFLIGHT = new Map();
...
async function verifyAccessTokenWithSupabase(token, source) {
  ...
  let userPromise = VERIFIED_TOKEN_INFLIGHT.get(normalizedToken);
  if (!userPromise) {
    userPromise = withTimeout(client.auth.getUser(normalizedToken), SUPABASE_FALLBACK_TIMEOUT_MS);
    VERIFIED_TOKEN_INFLIGHT.set(normalizedToken, userPromise);
  }
  ...
  try {
    ...
    if (VERIFIED_TOKEN_INFLIGHT.get(normalizedToken) === userPromise) {
      VERIFIED_TOKEN_INFLIGHT.delete(normalizedToken);
    }
  }
...
export async function requireProtectedSession(request, options = {}) {
  const sessionContext = await requireSessionRequest(request, options);
  return assertSessionNotRevoked(sessionContext);
}
