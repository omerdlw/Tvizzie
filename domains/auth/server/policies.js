...
import { AUTH_COOKIE_PATH, isSecureCookieEnvironment, requireProtectedSession } from './session';
import { ACCOUNT_LIFECYCLE_STATES, assertAccountLifecycleAllowed } from './account';
import { requireHighRiskAal2 } from './security-surfaces';
...
export async function requirePolicySession(request, policyKey) {
  ...
  const sessionContext = await requireProtectedSession(request, {
  ...
  if (policy.highRisk) await requireHighRiskAal2(sessionContext);
