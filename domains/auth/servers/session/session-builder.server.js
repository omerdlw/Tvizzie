...
import { RESERVED_CLAIM_KEYS } from '@/domains/auth/utils';
import { normalizeValue, toIsoDate, toLowercase } from './session-shared';

function resolveCustomClaims(decodedToken = {}) {
  return Object.fromEntries(
    Object.entries(decodedToken || {}).filter(([key]) => !RESERVED_CLAIM_KEYS.has(key)),
  );
}
...
export function buildNormalizedSession(decodedToken = {}, userRecord = null) {
  ...
  return {
    ...
    provider:
      capabilities.primaryProvider || resolvePrimaryProvider(user?.metadata?.providerIds || []),
    user,
  };
}
