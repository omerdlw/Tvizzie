...
const FOLLOWING_SELECT = 'following_avatar_url,following_display_name,following_id,following_username';
...
export async function getAccountSocialProofResource({ canViewPrivateContent, targetUserId, viewerId }) {
  ...
  if (viewerLikesKeys.length > 0) {
    const { count: sharedLikesCount, data: sharedLikesData, error: sharedError } = await admin
      .from('likes')
...
export async function GET(request) {
  try {
    ...
    const resource = normalizeTrim(searchParams.get('resource'));
    const viewerId = authContext?.userId || null;
    const targetUserId = normalizeTrim(searchParams.get('targetUserId'));
    const entityId = normalizeTrim(searchParams.get('entityId'));
    const entityType = normalizeTrim(searchParams.get('entityType'));
