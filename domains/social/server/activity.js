...
function buildReviewCardSnapshot({ actor, occurredAt, payload = {}, subject }) {
  ...
  return {
    ...
    subjectKey: normalizeValue(
      payload.subjectKey ||
        buildActivitySubjectRef({ subjectId: subject.id, subjectType: subject.type }),
    ),
...
export function buildEventRecord({ actor, eventType, occurredAt, payload = {}, visibility }) {
  ...
  switch (eventType) {
    ...
    case ACTIVITY_EVENT_TYPES.WATCH_DIARY_REWATCHED: {
      const diaryEntryId = normalizeValue(payload.diaryEntryId || payload.entryId);
      if (!diaryEntryId) {
        throw new Error('invalid-event-payload: diary entry id is required');
      }

      return {
        details: {
          diaryEntryId,
          entryKind: normalizeValue(payload.entryKind) || 'title',
          episodeNumber: normalizeOptionalNumber(payload.episodeNumber),
          episodeTitle: normalizeValue(payload.episodeTitle) || null,
          isRewatch: eventType === ACTIVITY_EVENT_TYPES.WATCH_DIARY_REWATCHED,
          seasonNumber: normalizeOptionalNumber(payload.seasonNumber),
          watchedAt: normalizeValue(payload.watchedAt) || occurredAt,
        },
        eventType,
        occurredAt,
