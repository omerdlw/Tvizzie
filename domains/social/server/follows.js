...
const FOLLOW_EVENT_PUBLISH_TIMEOUT_MS = 1500;
...
export async function publishFollowChange({
  followerId,
  followingId,
  reason,
  status = null,
  ...
  await Promise.race([
    publishPromise,
    new Promise((resolve) => setTimeout(resolve, FOLLOW_EVENT_PUBLISH_TIMEOUT_MS)),
  ]);
}
...
export async function getFollowResource({
  ...
  if (resource === 'followers-count' || resource === 'following-count') {
    ...
    if (
      ...
      const error = new Error('You are not allowed to view this follow count');
      error.status = 403;
      throw error;
    }

    const direction = resource === 'followers-count' ? 'followers' : 'following';
    const baseColumn = direction === 'followers' ? 'following_id' : 'follower_id';
    let query = admin
      .from('follows')
      .select(baseColumn, { count: 'exact', head: true })
      .eq(baseColumn, userId);
    ...
    const result = await executeCollectionQuery(query, {
      fallbackValue: { count: 0, error: null },
      label: `${resource} for user ${userId}`,
      strict,
    });

    if (result?.timedOut) return 0;

    assertResult(result, 'Follow count could not be loaded');
