...
export function fireActivityEvent(eventType, payload = {}) {
  const request = postActivityEvent({ eventType, payload });

  void request.catch((error) => {
    console.error('[Activity] Event delivery failed:', {
      eventType,
      message: error?.message || 'Unknown activity delivery error',
...
export async function fetchAccountActivityFeed({
  ...
  return requestAccountRead('/api/account/activity', {
    cursor,
    pageSize,
    scope,
    sort,
