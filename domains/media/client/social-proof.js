...
function normalizeKnownMovieIds(value = []) {
  ...
  return Array.from(
    new Set(
      value.map((item) => String(item?.id ?? item?.entityId ?? item ?? '').trim()).filter(Boolean),
    ),
  ).slice(0, 30);
}

export async function fetchMediaSocialProof({ media, viewerId, knownMovieIds = [] }) {
  if (!viewerId || !media) {
  ...
  const data = await requestApiJson('/api/social-proof', {
    query: {
  ...
  return data?.data || createEmptyMediaSocialProof();
}

export function subscribeToMediaSocialProof(
  { media, viewerId, knownMovieIds = [] },
  callback,
  options = {},
) {
  return createPollingSubscription(
    () => fetchMediaSocialProof({ media, viewerId, knownMovieIds }),
    callback,
    {
      ...options,
...
export async function getAccountSocialProof({
  ...
  return data?.data || createEmptyProfileSocialProof();
}
