  ...
  return Array.from(
    new Set(
      keywords
        ...
        .filter(
          (keyword) =>
            keyword && keyword.length <= 28 && !genreSet.has(normalizeComparableText(keyword)),
        ),
    ),
...
function getGenreIds(movie = {}) {
  ...
  return Array.from(
    new Set([...genreIds, ...genres].map((value) => Number(value)).filter(Number.isFinite)),
  );
}
...
function scoreRelatedMovie(baseMovie, candidate, mode = 'recommendation') {
  ...
  const yearDiff =
    Number.isFinite(baseYear) && Number.isFinite(candidateYear)
      ? Math.abs(candidateYear - baseYear)
      : null;
