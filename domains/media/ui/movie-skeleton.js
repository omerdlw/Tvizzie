...
function TextLine({ width = 'w-full', soft = false, className = 'h-4' }) {
  const size =
    className === 'h-2.5' ? 'xs' : className === 'h-3' ? 'sm' : className === 'h-3.5' ? 'md' : 'lg';
  return <SkeletonLine size={size} className={width} soft={soft} />;
}

function SegmentTabs() {
  return (
    <div className="skeleton-block-soft inline-flex w-fit items-center" style={{ padding: '2px', borderRadius: '12px' }}>
