          ...
          <div className="flex flex-col gap-8">
            {Array.from({ length: 4 }).map((_, yearIndex) => (
              <div key={yearIndex} className="relative flex">
                ...
                {/* Timeline Dot */}
                <div className="absolute top-[18px] left-16 z-10 size-3 -translate-x-1/2 rounded-full border-2 border-white bg-black/30 sm:left-24" />

                <div className="min-w-0 flex-1 space-y-3 pt-1 pl-4 sm:pl-8">
                  {Array.from({ length: yearIndex === 0 ? 2 : 1 }).map((_, itemIndex) => (
                    <div
                      key={itemIndex}
                      className="flex items-center gap-4 rounded-[20px] p-2"
                      style={{ animationDelay: `${(yearIndex * 2 + itemIndex) * 50}ms` }}
                    >
                      <div className={`h-24 w-16 shrink-0 rounded-2xl ${S} sm:w-20`} />
