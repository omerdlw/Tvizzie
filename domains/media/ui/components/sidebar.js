...
function SidebarRow({ icon, children }) {
  return (
    <div className="flex items-start gap-2.5 py-1.5 text-xs text-white last:pb-0 sm:text-sm">
...
function SidebarTaxonomy({ genres = [], tags = [] }) {
  ...
  const elements = (
    <div className="mt-2 flex flex-col gap-2.5">
      <div>
        <p className="text-xs leading-none font-semibold text-white/70 uppercase">GENRES / TAGS</p>
      </div>
...
export default function Sidebar({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative aspect-2/3 w-full shrink-0 overflow-hidden rounded-[20px]">
        {posterSrc ? (
          <AdaptiveImage
            fill
            priority
      ...
      {!hideTaxonomy && hasTaxonomy ? taxonomyData.element : null}

      {!hideMetadata && rows.length > 0 && (
