...
export const MEDIA_BACKGROUND_ANIMATION = Object.freeze({
  initial: { opacity: 0, scale: 1.10, filter: BLURS.CINEMATIC },
  animate: { opacity: 1, scale: 1, filter: BLURS.NONE },
  exit: { opacity: 0, scale: 0.90, filter: BLURS.MEDIUM },
  transition: {
...
export const PERSON_TIMELINES = Object.freeze({
  HERO_TITLE_DELAY: 0.10,
  BIO_DELAY: 0.30,
  GALLERY_BASE_DELAY: 0.55,
  FILMOGRAPHY_BASE_DELAY: 0.85,
  TIMELINE_BASE_DELAY: 0.40,
  AWARDS_BASE_DELAY: 0.40,
});
...
export function getCastHeaderProps(baseDelay = TIMELINES.CAST_SECTION_BASE_DELAY, hasSwitchedTab = false) {
  return {
    initial: {
      opacity: 0,
...
export function getCastCardProps(index = 0, baseDelay = TIMELINES.CAST_SECTION_BASE_DELAY, hasSwitchedTab = false) {
  const cardDelay = hasSwitchedTab
    ? index * TIMELINES.TAB_SWITCH_CARD_STEP
    : baseDelay + 0.10 + index * TIMELINES.CAST_CARD_STEP;
...
export function getMediaCardProps(index = 0, baseDelay = 0, hasSwitchedTab = false) {
  const cardDelay = hasSwitchedTab
    ? index * TIMELINES.TAB_SWITCH_CARD_STEP
    : baseDelay + 0.10 + index * TIMELINES.GALLERY_CARD_STEP;
  const cardDuration = hasSwitchedTab ? DURATIONS.TAB_SWITCH_CARD : DURATIONS.CARD_REVEAL;
