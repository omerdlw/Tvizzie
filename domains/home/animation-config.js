...
export function getDiscoverCardProps(index = 0) {
  return {
    initial: { opacity: 0, y: 28, scale: SCALES.HERO, filter: BLURS.MEDIUM },
    animate: { opacity: 1, y: 0, scale: 1, filter: BLURS.NONE },
    transition: {
      duration: DURATIONS.GRID_CARD,
      delay: Math.min(index * TIMELINES.DISCOVER_GRID_STEP, 0.32),
      ease: EASINGS.LUXURY,
    },
...
export function getTrendingCardProps(index = 0) {
  return {
    initial: { opacity: 0, y: 24, scale: SCALES.HERO, filter: BLURS.MEDIUM },
    animate: { opacity: 1, y: 0, scale: 1, filter: BLURS.NONE },
    transition: {
      duration: DURATIONS.RAIL_CARD,
      delay: Math.min(index * TIMELINES.RAIL_CARD_STEP, 0.3),
      ease: EASINGS.LUXURY,
    },
