...
import { deleteUserList } from '@/domains/account/client/lists';
import {
  FOLLOW_STATUSES,
  cancelFollowRequest,
  followUser,
...
export function useAccountPageActions({
  ...
  const handleFollow = useCallback(async () => {
    ...
    try {
      if (isCancelRequest) {
        ...
        await followUser(auth.user.id, profile.id, { isPrivate: isTargetPrivate });
      }
    } catch (error) {
      if (typeof setFollowRelationship === 'function') {
        setFollowRelationship(previousRelationship);
  ...
  const handleOpenFollowList = useCallback(
    ...
    [canViewPrivateContent, isOwner, isPrivateProfile, openSurface, profile, resolvedUserId],
  );
