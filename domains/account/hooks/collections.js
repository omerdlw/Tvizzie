...
import { getLikeDocRef, removeUserLike, subscribeToUserLikes } from '@/domains/media/client/likes';
import {
  toggleUserListItem,
...
import { removeUserWatchedItem, subscribeToUserWatched } from '@/domains/media/client/watched';
import { updateUserMediaPosition } from '@/domains/media/utils/poster-preferences';
...
function subscribeToAccountCollection({
  ...
  return config.subscribe(
    ...
    },
    {
      ...
      ...(hasSeededCollectionItems ? { initialPayload: seededItems } : {}),
      limitCount,
      onError: (error) => {
        if (isDisposed()) return;
        setLoading(false);
