...
import { useNavigationActions, useRegisterBreadcrumbOverride } from '@/modules/nav';
...
export default function AccountListDetailView({ routeData = null }) {
  ...
  const currentUsername = resolvedRouteData?.username || profile?.username || '';
  const currentListSlug = slug || list?.slug || '';
  const currentPath = currentUsername && currentListSlug ? `/account/${currentUsername}/lists/${currentListSlug}` : null;
  ...
  const handleBulkDeleteListItems = useCallback(
    async (mediaKeys = []) => {
      if (!isOwner || !list?.id || !auth.user?.id || mediaKeys.length === 0) {
        return;
      }
      ...
      setListItems((currentItems) => {
        previousListItems = currentItems;
        nextListItems = currentItems.filter((item) => {
          const key = item?.mediaKey || item?.media_key || item?.id;
          return !mediaKeySet.has(key);
        });
        return nextListItems;
      });

      try {
        await deleteUserListItems({
          listId: list.id,
          mediaKeys,
          userId: auth.user.id,
        });
    ...
    [auth.user?.id, isOwner, list?.id, primeListItemsCache, toast],
  );
