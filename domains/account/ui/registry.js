...
export function buildAccountEditState({
  ...
  const shouldShowEditAction = activeTab === 'general' && isGeneralAccountDirty;
  const editNavAction = !authIsAuthenticated ? (
    <AccountAction isAuthenticated={false} onSignIn={handleSignIn} />
  ) : isMediaUploading || !shouldShowEditAction ? null : (
    <AccountAction
      mode="profile-edit"
...
export function buildAccountPageState({
  ...
  const showSectionSaveAction =
    isSectionEditing && isSectionOrderDirty && typeof onSaveSectionOrder === 'function';
  ...
  const hasOwnerAction =
    ...
    (canManageRequests &&
      ...
      typeof handleOpenFollowList === 'function') ||
    typeof onOpenReviewComposer === 'function' ||
    Boolean(ownReview) ||
    (!profile && !isResolvingProfile && (Boolean(username) || Boolean(resolveError)));

  const shouldRenderDefaultAccountAction = !isOwner || hasOwnerAction;

  return {
    loading: loadingState,
    nav: {
      path: !isOwner && username ? `/account/${username}` : '/account',
      actions: accountNavActions,
      description: navDescription,
      icon: getUserAvatarUrl(profile),
      iconOverlay: profile?.isPrivate
        ? { icon: 'solar:lock-keyhole-bold', title: 'Private profile' }
        : shouldShowCurrentAccountAvatar
          ? {
      ...
      ) : shouldRenderDefaultAccountAction ? (
        <AccountAction
          isOwner={isOwner}
...
export const AccountEditRegistry = createRouteRegistry({
