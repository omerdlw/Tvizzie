...
import { SUPABASE_PUBLISHABLE_KEY, SUPABASE_URL } from '@/infrastructure/supabase/constants';
...
async function ensureBucket(client, bucket) {
    ...
    if (existingBucket.data?.public !== true) {
      ...
      if (updateResult.error) {
        throw createHttpError(
          updateResult.error.message || 'Storage bucket could not be updated',
          500,
        );
      }
    }
  ...
  if (errorMessage && !errorMessage.includes('not found')) {
    throw createHttpError(
      existingBucket.error.message || 'Storage bucket could not be checked',
      500,
    );
  }
  ...
  if (createResult.error) {
    ...
    if (!createMessage.includes('already')) {
      throw createHttpError(
        createResult.error.message || 'Storage bucket could not be created',
        500,
      );
    }
  }
