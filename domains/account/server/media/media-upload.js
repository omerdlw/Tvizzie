...
import { getRequestContext, requireSessionRequest } from '@/domains/auth/servers/auth-session.js';
import { buildInternalRequestMeta, executeWriteRollout } from '@/infrastructure/http/http-server';
...
export async function handleAccountMediaPost(request) {
  try {
    ...
    const { fileBuffer, fileExtension, fileSize, mimeType, target } =
      await readAccountMediaUploadPayload(request);
