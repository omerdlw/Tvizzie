...
import { createAppError } from '@/infrastructure/http/server';
import { createAdminClient } from '@/infrastructure/supabase/server';
import { canViewerAccessUserContent, createPrivateProfileError } from './profile';
...
function normalizeOverviewListRows(rows = []) {
  const normalizedRows = Array.isArray(rows) ? rows : [];
  const likesMap = new Map(
    normalizedRows
      .filter((row) => row?.id)
      .map((row) => [row.id, Number(row.likes_count)])
      .filter(([, count]) => Number.isFinite(count)),
  );

  return normalizedRows.map((row) => normalizeListRow(row, likesMap));
}

export async function getAccountOverviewCollections({
  admin: customAdmin,
  limits = {},
  strict = false,
  timeoutMs = 4000,
  ...
  const result = await executeCollectionQuery(
    admin.rpc('get_account_overview_collections', {
      p_likes_limit: resolveLimitCount(limits.likes, 12, 50),
      p_lists_limit: resolveLimitCount(limits.lists, 6, 50),
      p_user_id: userId,
      p_watched_limit: resolveLimitCount(limits.watched, 12, 50),
      p_watchlist_limit: resolveLimitCount(limits.watchlist, 12, 50),
    }),
    {
      fallbackValue: { data: null, error: null },
      label: 'Account overview collections for user ' + userId,
      strict,
      timeoutMs,
    },
  );
  ...
  assertResult(result, 'Account overview collections could not be loaded');
  ...
  const likes = Array.isArray(data.likes)
    ? data.likes
        ...
        .map(toAccountOverviewMediaItem)
    : [];
  const lists = normalizeOverviewListRows(data.lists);
  const likedLists = normalizeOverviewListRows(data.likedLists);
  const watched = Array.isArray(data.watched)
    ? data.watched.map(normalizeWatchedRow).map(toAccountOverviewMediaItem)
    : [];
...
function toAccountOverviewMediaItem(item = {}) {
  return {
    ...
    id: item.id || item.entityId || null,
    lastWatchedAt: item.lastWatchedAt || null,
    mediaKey: item.mediaKey || null,
    media_type: item.media_type || item.entityType || null,
    name: item.name || '',
...
export async function toggleListLike({ listId, ownerId, userId }) {
  const normalizedListId = normalizeValue(listId);
  const normalizedOwnerId = normalizeValue(ownerId);
  const normalizedUserId = normalizeValue(userId);

  if (!normalizedListId || !normalizedOwnerId || !normalizedUserId) {
    throw createAppError('listId, ownerId, and userId are required', {
      code: 'INVALID_LIST_LIKE_REQUEST',
      status: 400,
    });
  }

  if (normalizedOwnerId === normalizedUserId) {
    throw createAppError('You cannot like your own list', {
      code: 'OWN_LIST_LIKE_NOT_ALLOWED',
      status: 400,
    });
  }
  ...
  if (listResult.error) {
    throw createAppError(listResult.error.message || 'List could not be loaded', {
  ...
  if (!listResult.data) {
    throw createAppError('List not found', {
      code: 'LIST_NOT_FOUND',
      status: 404,
    });
  }
  ...
  if (rpcResult.error) {
    throw createAppError(rpcResult.error.message || 'List like state could not be updated', {
      code: 'LIST_LIKE_UPDATE_FAILED',
      status: 500,
    });
  }
  ...
  return {
    ...
    list: {
      ...
      slug: list.slug || normalizedListId,
