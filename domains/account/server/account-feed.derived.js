...
import { getAccountProfileByUserId } from '@/domains/account/server/account-profile.server';
import { fetchProfileReviewFeedServer } from '@/domains/reviews/server/server.js';
...
function createDerivedReviewCard(review = {}) {
  return normalizeReviewCard({
    ...
    subjectPreviewItems: Array.isArray(review?.subjectPreviewItems)
...
export async function fetchDerivedUserActivityItems({
  ...
  const derivedItems = [
    ...(Array.isArray(likes) ? likes : []).map((item) =>
      createDerivedCollectionActivityItem('likes', item, actor),
    ),
    ...
    ...(Array.isArray(lists) ? lists : []).map((item) =>
      createDerivedCollectionActivityItem('lists', item, actor),
    ),
