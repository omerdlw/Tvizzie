...
import { cleanString, normalizeTimestamp } from '@/shared/normalize';
...
export async function updateAccountProfile({ email, input = {}, userId }) {
  ...
  if (newUsername) {
    await claimUsernameForProfile({
      avatarUrl: input.avatarUrl !== undefined ? normalizeValue(input.avatarUrl) || null : null,
  ...
  if (input.avatarUrl !== undefined) updates.avatar_url = normalizeValue(input.avatarUrl) || null;
  if (input.bannerUrl !== undefined) updates.banner_url = normalizeValue(input.bannerUrl) || null;
  if (input.description !== undefined) updates.description = normalizeValue(input.description);
  if (input.isPrivate !== undefined) updates.is_private = Boolean(input.isPrivate);
  if (input.favoriteShowcase !== undefined) {
    ...
    updates.favorite_showcase = normalizeFavoriteShowcaseItems(input.favoriteShowcase);
  }
  ...
  if (updateResult.error) {
    console.error('Account update failed:', updateResult.error);
  ...
  await publishUserEvent(normalizedUserId, 'account:updated', { profile });
  return profile;
}
