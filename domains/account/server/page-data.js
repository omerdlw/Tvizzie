...
import { getAccountOverviewCollections, getAccountResource } from './collections';
import { getWatchDiaryStartMonth, getWatchTrackingResource } from './watch-tracking';
import { fetchAccountActivityFeedServer } from './feed';
import {
  fetchListReviewFeedServer,
  fetchProfileReviewFeedServer,
} from '@/domains/reviews/server/feeds';
import { getFollowResource } from '@/domains/social/server/follows';
import {
  ACCOUNT_OVERVIEW_DEFERRED_LOAD_TIMEOUT_MS,
...
function createInitialFeed(feed = null, resolvedUserId = null, extras = null) {
  ...
  return {
    error: feed?.loadError ? 'Account feed could not be loaded' : feed?.error || null,
...
export const getUsernameAccountSnapshot = cache(async (username) => {
  ...
  const [profile, initialFollowRelationship] = await Promise.all([
    // The page snapshot is a read path; reuse the short-lived server cache so
    // the /account redirect and the profile route do not repeat the same
    // profile/counter queries back-to-back.
    getAccountProfileByUserId(resolvedUserId, { bypassCache: false, viewerId }),
...
async function loadAccountResource(
  ...
  try {
    const isPrimaryListRead = PRIMARY_LIST_READ_RESOURCES.has(resource);
    const result = await withTimeout(
      getAccountResource({
        ...
        ...(slug ? { slug } : {}),
        resource,
        strict: isPrimaryListRead,
        userId,
        viewerId: snapshot?.viewerId || null,
      }),
      isPrimaryListRead ? 12000 : undefined,
    );
...
function getCurrentDiaryMonthBounds(date = new Date()) {
  ...
  return {
    ...
    monthKey: firstDay.toISOString().slice(0, 7),
    toDate: lastDay.toISOString().slice(0, 10),
  };
}

async function loadWatchDiaryResource(snapshot = null, { fromDate = null, toDate = null } = {}) {
  const userId = resolveSnapshotUserId(snapshot);
  if (!userId) return [];
...
async function loadWatchDiaryStartMonth(snapshot = null) {
  const userId = resolveSnapshotUserId(snapshot);
  if (!userId) return null;

  return safeLoad(
    () =>
      getWatchDiaryStartMonth({
        userId,
        viewerId: snapshot?.viewerId || null,
      }),
    null,
...
async function loadOverviewCollections(snapshot = null) {
  ...
  const canViewCollections =
    snapshot?.viewerId === userId ||
    snapshot?.initialProfile?.isPrivate !== true ||
    snapshot?.initialFollowRelationship?.canViewPrivateContent === true;

  if (!canViewCollections) {
    return { likedLists: [], likes: [], lists: [], watched: [], watchlist: [] };
  }

  const aggregateCollections = await getAccountOverviewCollections({
    limits: {
      likes: OVERVIEW_LIKES_LIMIT,
      lists: OVERVIEW_LISTS_LIMIT,
      watched: OVERVIEW_WATCHED_LIMIT,
      watchlist: OVERVIEW_WATCHLIST_LIMIT,
    },
    timeoutMs: ACCOUNT_OVERVIEW_DEFERRED_LOAD_TIMEOUT_MS,
    userId,
  }).catch(() => null);

  if (aggregateCollections) return aggregateCollections;
  return null;
}
...
export async function getUsernameAccountOverviewRouteData(username) {
  ...
  const [overviewCollections, rawActivityFeed, rawReviewFeed] = await Promise.all([
  ...
  return createRouteState(snapshot, {
    ...
    initialCollections: overviewCollections
      ? createSnapshotInitialCollections(snapshot, overviewCollections)
      : null,
    initialReviewFeed: createInitialFeed(rawReviewFeed, snapshot.initialResolvedUserId, {
      mode: 'authored',
    }),
    username,
  });
...
export async function getUsernameAccountDiaryRouteData(username) {
  const snapshot = await getUsernameAccountSnapshot(username);
  if (!snapshot.initialResolvedUserId) return createMissingUsernameRouteState(snapshot, username);

  const bounds = getCurrentDiaryMonthBounds();
  const [entries, firstMonthKey] = await Promise.all([
    loadWatchDiaryResource(snapshot, bounds),
    loadWatchDiaryStartMonth(snapshot),
  ]);
