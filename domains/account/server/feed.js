...
import { ACTIVITY_EVENT_TYPES, ACTIVITY_EVENT_TYPE_SET } from '@/domains/social/utils/constants';
import {
  canViewerAccessUserContent,
  createPrivateProfileError,
  getAccountProfileByUserId,
} from './profile';
import { getAccountResource } from './collections';
...
import { normalizeMediaType } from '@/domains/media/utils/media-key';
import { chunkArray, normalizeTimestamp, normalizeValue } from '@/shared';
...
function getActivityDeduplicationKey(item = {}) {
  ...
  if (
    (eventType === ACTIVITY_EVENT_TYPES.WATCH_DIARY_LOGGED ||
      eventType === ACTIVITY_EVENT_TYPES.WATCH_DIARY_REWATCHED) &&
    normalizeValue(item?.dedupeKey)
  ) {
    return normalizeValue(item.dedupeKey);
  }

  if (eventType === ACTIVITY_EVENT_TYPES.REVIEW_LIKED && normalizeValue(item?.dedupeKey)) {
    return normalizeValue(item.dedupeKey);
  }
...
function projectActivityLine(item = {}, viewerId = null) {
  ...
  switch (item.eventType) {
    case ACTIVITY_EVENT_TYPES.WATCH_DIARY_LOGGED:
      return { parts: [actorPart, createTextPart(' logged '), subjectPart] };
    case ACTIVITY_EVENT_TYPES.WATCH_DIARY_REWATCHED:
      return { parts: [actorPart, createTextPart(' rewatched '), subjectPart] };
...
export async function fetchDerivedUserActivityItems({
  ...
  const [profile, likes, watchlist, watched, lists, likedLists, reviewFeed] = await Promise.all([
    getAccountProfileByUserId(userId, { bypassCache: false, viewerId }).catch(() => null),
    getAccountResource({
