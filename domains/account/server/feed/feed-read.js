...
import { ACTIVITY_SELECT, FOLLOW_STATUS_ACCEPTED } from '@/domains/account/utils';
...
import { projectActivityItem } from './feed-projector';

async function fetchActivitiesForSources(admin, sourceIds = [], pageSize = null) {
  const uniqueSourceIds = [
    ...new Set(sourceIds.map((value) => normalizeValue(value)).filter(Boolean)),
  ];
  ...
  const perSourceLimit =
    Number.isFinite(Number(pageSize)) && Number(pageSize) > 0 ? Number(pageSize) : null;
...
export async function fetchAccountActivityFeedServer({
  ...
  const sourcePageSize = shouldResolveFullFilteredSet
    ? null
    : Math.max(normalizedOffset + normalizedPageSize * 2, 24);
  ...
  const rawActivityItems = (await fetchActivitiesForSources(admin, sourceIds, sourcePageSize)).map(
    (item) => ({
      ...item,
  ...
  const items = sortActivityItemsForMode(
    filterActivityItemsBySubject(
      dedupeActivityItems(
        rawActivityItems.length > 0 ? rawActivityItems : derivedUserActivityItems,
      ),
      normalizedSubject,
    ),
    normalizedSort,
