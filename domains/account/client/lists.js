...
import { cleanString, chunkArray, normalizeTimestamp } from '@/shared/normalize';
import {
...
export async function createUserListWithItems({
  ...
  const insertResult = await client.rpc('list_create_with_items_atomic', {
  ...
  const createdRow = resolveRpcRow(insertResult.data);
