...
import WatchedView from '@/domains/account/screens/account-watched-page';
...
function useWatchedClientState({
  ...
  const { canViewProfileCollections, isOwner, isPrivateProfile, resolvedUserId, isPageLoading } =
    sectionState;
    ...
    return subscribeToUserWatched(
      ...
      (nextItems) => {
        setWatchedItems((currentItems) =>
          mergeCollectionItemsWithExistingMetadata(currentItems, nextItems),
        );
        setLoadError(null);
        setIsWatchedLoading(false);
      },
