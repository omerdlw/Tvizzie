...
import { rankAllMediaResults, resolvePreferredMediaType } from '@/domains/search/utils/ranking';
import { normalizeResult } from '@/domains/search/utils/result';
...
function resolvePreferredTitleType({ movieResults = [], normalizedQuery = '', tvResults = [] }) {
  ...
  if (exactMovies.length && exactTv.length) {
    const topMovie = [...exactMovies].sort(
      (left, right) => getTitleAuthority(right) - getTitleAuthority(left),
    )[0];
    const topTv = [...exactTv].sort(
      (left, right) => getTitleAuthority(right) - getTitleAuthority(left),
    )[0];

    return getTitleAuthority(topTv) > getTitleAuthority(topMovie)
      ? SEARCH_TYPES.TV
      : SEARCH_TYPES.MOVIE;
  }
...
export async function fetchMediaPage(query, type, page = 1, options = {}) {
  ...
  return withClientSearchCache(cacheKey, async () => {
    try {
      const { TmdbService } = await import('@/infrastructure/tmdb/client');
...
export async function fetchAllMedia(query, page = 1, options = {}) {
  ...
  return rankAllMediaResults(
    [...moviePayload.results, ...tvPayload.results],
