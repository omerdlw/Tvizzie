...
function getBestMovieTitleMatchScore(movie = {}, normalizedQuery = '', queryTokens = []) {
  ...
  const titles = [movie?.title, movie?.original_title, movie?.name, movie?.original_name].filter(
    Boolean,
  );
  let bestScore = 0;
  ...
  titles.forEach((title) => {
    ...
    if (normalizedTitle === normalizedQuery || articleStrippedTitle === normalizedQuery) {
      ...
      articleStrippedTitle.startsWith(normalizedQuery)
    ) {
      score += 10;
...
export function resolvePreferredMediaType({
  movieResults = [],
  personResults = [],
  query = '',
  tvResults = [],
  ...
  const topTitleResult =
    !topTvResult ||
    (topMovieResult && topMovieResult.score.totalScore >= topTvResult.score.totalScore)
      ? topMovieResult
      : topTvResult;
  ...
  const strongMoviePrefixMatchCount = countStrongMoviePrefixMatches(
    movieResults,
    normalizedQuery,
    queryTokens,
  );
  const strongTvPrefixMatchCount = countStrongMoviePrefixMatches(
    tvResults,
    normalizedQuery,
    queryTokens,
  );
  const hasExactMovieMatch = movieResults.some((movie) =>
    isExactMovieTitleMatch(movie, normalizedQuery),
  );
  ...
  if (
    ...
    (topCombinedTitleScore.textScore >= 14 &&
      topCombinedTitleScore.totalScore >= topPersonScore.totalScore + 2)
  ) {
    return topTitleResult?.item?.media_type === SEARCH_TYPES.TV
      ? SEARCH_TYPES.TV
      : SEARCH_TYPES.MOVIE;
  }

  const hasExactPersonMatch = personResults.some((person) =>
    isExactPersonNameMatch(person, normalizedQuery),
  );
  ...
  const directTitleResults = movieResults.map((movie, index) => {
    ...
    return {
      ...
      __searchRankScore:
        score.totalScore * 40 +
        score.textScore * 18 +
        score.authorityScore * 6 +
        Math.max(0, 200 - index),
    };
  });
