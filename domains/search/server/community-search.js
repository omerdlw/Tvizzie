...
import { normalizeTimestamp } from '@/shared/normalize';
...
async function searchLists(admin, query, limit) {
  ...
  return rows
    .map((row) => normalizeListResult(row, ownerMap.get(row.user_id)))
...
async function searchListReviews(admin, query, limit) {
  ...
  return rows
    .map((row) =>
      normalizeReviewResult(row, {
        list: listMap.get(row.list_id),
...
async function loadCommunitySearch({ limit, query, type }) {
...
export async function searchCommunity({ limitCount, query: queryValue, type: typeValue }) {
  const query = normalizeValue(queryValue);
  const type = normalizeType(typeValue);
  const limit = normalizeLimit(limitCount);

  if (!query) {
    return { items: [], lists: [], reviews: [] };
  }
  ...
  const payload = await getOrLoadCachedValue({
    ...
    enabled: true,
    ttlMs: 1500,
    loader: () => loadCommunitySearch({ limit, query, type }),
  });
