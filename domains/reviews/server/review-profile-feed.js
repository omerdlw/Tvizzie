...
import { LIST_REVIEW_SELECT, MEDIA_REVIEW_SELECT, REVIEW_LIKE_SELECT } from '@/domains/reviews/utils';
...
async function fetchLikedReviews(admin, userId, { fetchLimit = null } = {}) {
  ...
  const mediaReviewUserIds = [
    ...new Set(mediaLikeRows.map((row) => row.review_user_id).filter(Boolean)),
  ];
  const listIds = [
    ...new Set(
      listLikeRows.map((row) => parseListReviewLikeKey(row.media_key)?.listId).filter(Boolean),
    ),
  ];
  const listReviewUserIds = [
    ...new Set(listLikeRows.map((row) => row.review_user_id).filter(Boolean)),
  ];
  ...
  const mediaReviews = mediaLikeRows
    .map((likeRow) => {
      ...
      return normalizeReviewRow(
        review,
        {},
        likesMap.get(`${likeRow.media_key}:${review.user_id}`) || [],
      );
    })
    .filter(Boolean);
