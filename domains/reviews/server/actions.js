...
import { invalidateCachedValuesWhere } from '@/infrastructure/http/server';
import { createMediaPayload } from '@/domains/media/utils/media-payload';
import { normalizeValue } from '@/shared';
import { normalizeRating } from '@/domains/reviews/utils/formatting';
import { REVIEW_MAX_LENGTH, REVIEW_MIN_LENGTH } from '@/domains/reviews/utils/constants';
...
async function toggleReviewLikeAction({ admin, body, userId }) {
  ...
  return {
    isNowLiked: Boolean(
      resultRow?.out_is_liked ??
      resultRow?.is_liked ??
      resultRow?.out_is_now_liked ??
      resultRow?.is_now_liked,
    ),
  };
}
...
export async function executeReviewWriteAction({ action, admin, body, userClient, userId }) {
  ...
  invalidateCachedValuesWhere((key) => key.startsWith('reviews|'));
