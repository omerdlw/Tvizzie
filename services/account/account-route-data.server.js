...
} from '@/services/media/reviews.server'
...
function createInitialCollections({
  ...
  return {
    counts: {
      likes: Number(counts?.likes ?? 0) || 0,
      lists: Number(counts?.lists ?? lists.length ?? 0) || 0,
      watched: Number(counts?.watched ?? watched.length ?? 0) || 0,
      watchlist: Number(counts?.watchlist ?? watchlist.length ?? 0) || 0,
    },
    likes: [],
