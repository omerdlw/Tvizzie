...
async function loadProfileCountersV2(userId) {
  ...
  const refreshResult = await admin
    .rpc('refresh_profile_counters_v2', {
...
const getAccountProfileV2 = cache(async (
  userId,
  { includeEmail = false, includePrivateDetails = false } = {}
) => {
  const normalizedUserId = normalizeValue(userId)
  ...
  const [counters, listsCount] = await Promise.all([
    loadProfileCountersV2(normalizedUserId).catch(() => null),
  ...
  return normalizeAccountData(
    {
      ...profileResult.data,
      follower_count: counters?.follower_count ?? 0,
...
export async function getAccountSnapshotByUserId(userId, options = {}) {
  try {
    const profile = await getAccountProfileV2(userId, options)
...
export async function getEditableAccountSnapshotByUserId(userId) {
  ...
  try {
    const profile = await getAccountProfileV2(normalizedUserId, {
      includeEmail: true,
