...
import { isListSubjectType, isMovieMediaType, isTvReference } from '@/lib/media'
...
async function fetchAuthoredReviews(admin, userId) {
  ...
  const mediaReviews = mediaRows.map((row) =>
    normalizeReviewRow(
      row,
      {},
      likesMap.get(`${row.media_key}:${row.user_id}`) || []
    )
  )
  ...
  const listReviews = listRows.map((row) => {
    ...
    return normalizeReviewRow(
      row,
      subject,
      likesMap.get(`${reviewKey}:${row.user_id}`) || []
    )
  })
...
export async function fetchListReviewFeedServer({
  listId,
  ownerId,
  viewerId = null,
}) {
