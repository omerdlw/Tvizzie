...
import { isListSubjectType, isMovieMediaType } from '@/lib/media'
import {
...
import { updateListReviewsCount } from '@/services/media/lists.service'
import {
...
function buildListSubjectMetadata({
  list = null,
  listId,
  ownerId,
  ownerUsername = null,
  ...
  const resolvedOwnerUsername =
    ownerUsername || list?.ownerSnapshot?.username || resolvedOwnerId
  ...
  return {
    ...
    subjectPoster:
      list?.coverUrl || list?.previewItems?.[0]?.poster_path || null,
...
function isMatchingMediaReviewEvent(payload = {}, media = null) {
  return (
    normalizeSubjectValue(payload?.subjectType) ===
      normalizeSubjectValue(media?.entityType) &&
...
export async function fetchUserMediaReviewMap({
  items = [],
  userId,
  username = null,
}) {
...
export function subscribeToListReviews(
  { list, ownerId, listId },
  callback,
  options = {}
) {
...
export async function upsertMediaReview({
  ...
  const existingPayload =
    existingResult.data?.payload && typeof existingResult.data.payload === 'object'
...
export async function upsertListReview({
  ...
  invalidatePollingSubscription(
    getListReviewsSubscriptionKey({ list, ownerId, listId }),
    {
      refetch: true,
    }
  )
...
export async function deleteListReview({ ownerId, listId, userId }) {
  ...
  invalidatePollingSubscription(
    getListReviewsSubscriptionKey({ list: null, ownerId, listId }),
    {
      refetch: true,
    }
  )
...
export async function toggleListReviewLike({
  ...
  if (!ownerId || !listId || !reviewUserId || !userId) {
    throw new Error(
      'ownerId, listId, reviewUserId, and userId are required to toggle a like'
    )
  }
  ...
  const listContext = isNowLiked
    ? await getListReviewContext(ownerId, listId)
    : null
...
export async function getListReviewContext(ownerId, listId) {
  ...
  const payload = result.data.payload && typeof result.data.payload === 'object'
    ? result.data.payload
    : {}
