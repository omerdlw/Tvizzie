...
export default function Client() {
  ...
  const isSignInBusy =
    isSubmitting || isPreparingReset || isIdentifierChecking
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (signInResult?.requiresVerification) {
        const didCompleteVerification =
          await handleLoginVerification(signInResult)
  ...
  const handleResetSubmit = async (event) => {
    ...
    try {
      ...
      setResetFlow(INITIAL_RESET_FLOW)
      toast.error(
        resolveAuthErrorMessage(error, 'Password reset could not be completed')
      )
    } finally {
