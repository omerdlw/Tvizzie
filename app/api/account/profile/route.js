...
} from '@/core/services/account/account.constants';
...
async function resolveAvailableUsername({ admin, displayName, email, preferredUsername, userId }) {
  if (
    ...
    normalizeValue(preferredUsername)
  ) {
...
async function writeLegacyProfile({
  ...
  const admin = createAdminClient();
  const currentSnapshot = await getEditableAccountSnapshotByUserId(normalizedUserId).catch(
    () => null,
  );
    ...
    const writeResult = await executeWriteRollout({
      ...
      edgeWrite: async () => {
        try {
          ...
          if (
            profileAfterError &&
            resolveProfileIsPrivate(profileAfterError) === requestedIsPrivate
          ) {
            return {
              action,
              ok: true,
