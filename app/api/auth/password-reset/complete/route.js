...
async function assertPasswordResetTargetIntegrity({
  userRecord,
  email,
}) {
  ...
  try {
    ...
    if (!passwordResetProof || !email) {
      return NextResponse.json(
        { error: 'passwordResetProof, email, and newPassword are required' },
        { status: 400 }
      )
    }
    ...
    const passwordResetVerification = verifyPasswordResetProofToken(
      passwordResetProof,
      { email }
    )
    ...
    if (
      ...
      String(challenge?.jti || '').trim() !==
        passwordResetVerification.challengeJti
    ) {
    ...
    await writeAuthAuditLog({
      ...
      console.error(
        '[AuthAudit] password-reset success log failed:',
        auditError
      )
    })
    ...
    await writeAuthAuditLog({
      ...
      console.error(
        '[AuthAudit] password-reset failure log failed:',
        auditError
      )
    })
