...
function Top250MovieCard({ item, index }) {
  ...
  return (
    <motion.div initial={itemMotion.initial} animate={itemMotion.animate} transition={itemMotion.transition}>
      <MediaCard
        href={href}
        className="w-full border border-black/10 bg-primary transition-transform duration-200 ease-out hover:scale-105 active:scale-95"
        imageSrc={posterSrc}
        imageAlt={item.title}
                  ...
                  <div className="account-detail-section-shell flex flex-col">
                    <div className="account-detail-section-body">
                      {items.length ? (
                        ...
                        <section className="border border-black/10 bg-primary px-4 py-5 text-sm font-medium text-black/60">
