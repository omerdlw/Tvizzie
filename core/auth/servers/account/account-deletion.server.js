...
export async function purgeAccountData({ userId }) {
  ...
  await executeDelete(
    admin.from('follows').delete().or(`follower_id.eq.${normalizedUserId},following_id.eq.${normalizedUserId}`),
  ...
  await executeDelete(admin.from('profiles').delete().eq('id', normalizedUserId), 'Profile could not be deleted');
}
