...
function resolveBrevoConfig() {
  ...
  const from =
    normalizeEnvValue(process.env.BREVO_SENDER_EMAIL) ||
...
async function sendWithBrevoApi({ html, subject, text, to, transportConfig }) {
  ...
  const providerMessage = normalizeEnvValue(
  ...
  throw new Error(
    providerMessage || `Email provider rejected the message with status ${response.status}`,
  );
}
