...
export async function resolveGoogleAuthIntent({
  ...
  const profileMatchesUser = !profile.exists || !profile.id || profile.id === normalizeValue(userId);
  const profileMatchesGoogleEmail = !profile.exists || !profile.email || profile.email === googleEmail;
