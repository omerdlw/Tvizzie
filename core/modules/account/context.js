...
function toAccountError(error, fallbackMessage) {
  ...
  const normalizedError = new Error(
    error?.message || fallbackMessage || 'Account request failed'
  )
...
export function AccountProvider({ children, config = EMPTY_OBJECT }) {
  ...
  const ensureCurrentAccount = useCallback(
    async (options = undefined) => {
      ...
      try {
        ...
        const normalizedError = toAccountError(
          error,
  ...
  const syncCurrentAccountEmail = useCallback(
    async (email) => {
      ...
      if (typeof adapter?.syncAccountEmail !== 'function') {
        throw new Error(
          'Account adapter method "syncAccountEmail" is not configured'
        )
      }
  ...
  useEffect(() => {
    ...
    if (
      mergedConfig.autoBootstrap === false ||
      typeof adapter?.ensureAccount !== 'function'
    ) {
      return undefined
    }
      ...
      try {
        const bootstrapPayload =
          typeof mergedConfig.bootstrap.resolvePayload === 'function'
            ...
            : null
        const nextAccount = await adapter.ensureAccount(
          auth.user,
        ...
        if (
          bootstrapPayload &&
          typeof mergedConfig.bootstrap.clearPayload === 'function'
        ) {
          await Promise.resolve(
            mergedConfig.bootstrap.clearPayload(
              auth.user,
              bootstrapPayload,
              nextAccount
            )
