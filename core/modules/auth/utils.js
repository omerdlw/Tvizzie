...
export function normalizeSession(input) {
  ...
  const capabilities = uniqueStrings([...user.capabilities, ...toArray(source.capabilities), ...permissions]);
  ...
  return {
    ...
    provider:
      source.provider || input.provider || capabilityState.primaryProvider || resolvePrimaryProvider(providerIds),
    metadata: {
      ...metadata,
...
export function mergeUserIntoSession(session, userPatch) {
  ...
  return normalizeSession({
    ...
    user: {
      ...
      ...mergedUser,
      permissions: uniqueStrings([...normalizedSession.user.permissions, ...mergedUser.permissions]),
      capabilities: uniqueStrings([...normalizedSession.user.capabilities, ...mergedUser.capabilities]),
