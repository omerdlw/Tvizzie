...
async function requestJson(url, { method = 'GET', body, headers = {}, withCredentials = true } = {}) {
  ...
  if (!response.ok) {
    throw createAuthAdapterError(
      payload?.message || payload?.error || `Authentication request failed with status ${response.status}`,
      response.status,
      payload
    );
  }
...
export function createApiAuthAdapter(options = {}) {
  ...
  async function callEndpoint(endpointKey, requestOptions = {}, context = {}) {
    ...
    if (!endpoint) {
      throw createAuthAdapterError(`API auth adapter endpoint "${endpointKey}" is not configured`);
    }

    const dynamicHeaders = typeof getHeaders === 'function' ? await getHeaders(endpointKey, context) : {};
  ...
  return createAuthAdapter({
    ...
    async signUp(payload, context = {}) {
      const responsePayload = await callEndpoint('signUp', { method: 'POST', body: payload }, context);
    ...
    async updateProfile(payload, context = {}) {
      if (!resolvedEndpoints.profile) {
        throw createAuthAdapterError('API auth adapter endpoint "profile" is not configured');
      }

      const responsePayload = await callEndpoint('profile', { method: 'PATCH', body: payload }, context);

      return mapResponse(responsePayload, 'updateProfile', context);
    },

    async requestPasswordReset(payload, context = {}) {
      return callEndpoint('requestPasswordReset', { method: 'POST', body: payload }, context);
    },

    async confirmPasswordReset(payload, context = {}) {
      return callEndpoint('confirmPasswordReset', { method: 'POST', body: payload }, context);
    },
  });
}
