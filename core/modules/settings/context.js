...
function createResolvedConfig(config = EMPTY_OBJECT, initialSettings = EMPTY_OBJECT) {
  const providedStorage = isObject(config.storage) ? config.storage : {};
  const providedLocalStorage = isObject(providedStorage.localStorage) ? providedStorage.localStorage : {};
  const providedCookie = isObject(providedStorage.cookie) ? providedStorage.cookie : {};
    ...
    storage: {
      ...
      cookie: {
        ...
        attributes: {
          ...
          ...(isObject(providedCookie.attributes) ? providedCookie.attributes : {}),
        },
      },
    },
  };
...
export function SettingsProvider({ children, config = EMPTY_OBJECT, initialSettings = EMPTY_OBJECT }) {
  const resolvedConfig = useMemo(() => createResolvedConfig(config, initialSettings), [config, initialSettings]);
  const storage = useMemo(() => createProviderStorage(resolvedConfig), [resolvedConfig]);
  ...
  const commitSettings = useCallback(
    (nextSettings, activeDefinitions = definitionsRef.current) => {
      const sanitizedSettings = sanitizeSettingsState(nextSettings, activeDefinitions);
  ...
  const setSetting = useCallback(
    (path, value) => {
      ...
      const nextSettings = setSettingValue(settingsRef.current, path, resolvedValue);
  ...
  const updateSettings = useCallback(
    (patch) => {
      const resolvedPatch = typeof patch === 'function' ? patch(settingsRef.current) : patch;
      const nextSettings = mergeSettingsObjects(settingsRef.current, isObject(resolvedPatch) ? resolvedPatch : {});
  ...
  const resetSettings = useCallback(
    (paths) => {
      const baseline = createSettingsBaseline(definitionsRef.current, initialSettingsRef.current);
  ...
  const registerSettings = useCallback(
    (nextDefinitions, { replace = false } = {}) => {
      const normalizedDefinitions = normalizeSettingsDefinitions(nextDefinitions);
  ...
  const stateValue = useMemo(
    ...
    [definitions, isHydrated, lastUpdatedAt, settings, resolvedConfig.storageKey]
  );
  ...
  return (
    <SettingsActionsContext.Provider value={actionsValue}>
      <SettingsStateContext.Provider value={stateValue}>{children}</SettingsStateContext.Provider>
...
export function useSetting(path, fallback = undefined) {
  ...
  return useMemo(() => getSettingValue(settings, path, fallback), [fallback, path, settings]);
}
