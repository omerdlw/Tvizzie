...
export function setSettingValue(source, path, value) {
  ...
  const base = Array.isArray(source) ? [...source] : isObject(source) ? { ...source } : {};

  base[segment] = rest.length === 0 ? cloneSettingValue(value) : setSettingValue(base[segment], rest, value);
...
export function removeSettingValue(source, path) {
  ...
  const removeRecursive = (current, index) => {
    if (current === null || current === undefined || (!isObject(current) && !Array.isArray(current))) {
      return current;
    }
...
function normalizeAdditionalStorageTargets(storage) {
  ...
  return Array.from(new Set(rawTargets.filter((target) => isString(target) && target.trim())));
}

function normalizeDefinition(path, definition) {
  const descriptor = isDefinitionDescriptor(definition) ? definition : { defaultValue: definition };

  const isPersistent = descriptor.persist !== false;
  const storage = isPersistent
    ? Array.from(
        new Set([SETTINGS_STORAGE_TARGETS.LOCAL_STORAGE, ...normalizeAdditionalStorageTargets(descriptor.storage)])
      )
    : [];
  ...
  return {
    ...
    options: Array.isArray(descriptor.options) ? descriptor.options.map((option) => ({ ...option })) : [],
...
export function normalizeSettingsDefinitions(definitions) {
  if (Array.isArray(definitions)) {
    return definitions.reduce((accumulator, definition) => {
      ...
      accumulator[definition.path] = normalizeDefinition(definition.path, definition);
      return accumulator;
    }, {});
  }
  ...
  return Object.entries(definitions).reduce((accumulator, [path, definition]) => {
...
export function buildSettingsDefaults(definitions = {}) {
  return Object.values(definitions).reduce((accumulator, definition) => {
    ...
    return setSettingValue(accumulator, definition.path, definition.defaultValue);
  }, {});
}

export function createSettingsBaseline(definitions = {}, initialSettings = {}) {
  return mergeSettingsObjects(buildSettingsDefaults(definitions), isObject(initialSettings) ? initialSettings : {});
}
...
export function sanitizeSettingsState(snapshot, definitions = {}) {
  ...
  Object.values(definitions).forEach((definition) => {
    const currentValue = getSettingValue(nextSnapshot, definition.path, SETTING_UNSET);
    ...
    nextSnapshot = setSettingValue(nextSnapshot, definition.path, definition.defaultValue);
  });
...
export function hydrateSettingsSnapshot(snapshot, definitions = {}) {
  ...
  Object.values(definitions).forEach((definition) => {
    const storedValue = getSettingValue(nextSnapshot, definition.path, SETTING_UNSET);

    if (storedValue === SETTING_UNSET) {
      return;
    }

    const hydratedValue = definition.deserialize ? definition.deserialize(storedValue) : storedValue;
...
export function createPersistedSettingsSnapshot(snapshot, definitions = {}) {
  ...
  Object.values(definitions).forEach((definition) => {
    ...
    if (definition.serialize) {
      nextSnapshot = setSettingValue(nextSnapshot, definition.path, definition.serialize(currentValue));
    }
  });
...
export function createCookieSettingsSnapshot(snapshot, definitions = {}) {
  const persistedSnapshot = createPersistedSettingsSnapshot(snapshot, definitions);
  ...
  return Object.values(definitions).reduce((accumulator, definition) => {
    ...
    const value = getSettingValue(persistedSnapshot, definition.path, SETTING_UNSET);
...
export function resolveSettingsSnapshot({ definitions = {}, initialSettings = {}, storedSettings = {} }) {
