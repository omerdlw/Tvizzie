...
const LIST_SELECT = ['created_at', 'description', 'id', 'likes_count', 'payload', 'poster_path', 'reviews_count', 'slug', 'title', 'updated_at', 'user_id'].join(',');
const MEDIA_REVIEW_SELECT = ['content', 'created_at', 'is_spoiler', 'media_key', 'payload', 'rating', 'updated_at', 'user_id'].join(',');
const LIST_REVIEW_SELECT = ['content', 'created_at', 'is_spoiler', 'list_id', 'payload', 'rating', 'updated_at', 'user_id'].join(',');
...
async function searchMediaReviews(admin, query, limit) {
  ...
  return rows.map((row) => normalizeReviewResult(row, { user: userMap.get(row.user_id) })).filter((item) => item.user?.id);
}
