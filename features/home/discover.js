...
export default function HomeDiscover({
  ...
  const [columnCount, setColumnCount] = useState(() =>
    typeof window === 'undefined' ? DEFAULT_DISCOVER_COLUMNS : getDiscoverColumnCount(window.innerWidth)
  );
  ...
  const [visibleCount, setVisibleCount] = useState(() => DEFAULT_DISCOVER_COLUMNS * DISCOVER_VISIBLE_ROWS);
  ...
  useEffect(() => {
    setVisibleCount((currentValue) => (currentValue < visibleBatchSize ? visibleBatchSize : currentValue));
  }, [visibleBatchSize]);
  ...
  const handleLoadMore = useCallback(async () => {
    if (visibleCount < items.length) {
      setVisibleCount((currentValue) => Math.min(currentValue + visibleBatchSize, items.length));
      return;
    }
    ...
    try {
      ...
      setItems(nextItems);
      setVisibleCount((currentValue) => Math.min(currentValue + visibleBatchSize, nextItems.length));
      setPage(nextPageInfo.page);
      setHasMore(nextPageInfo.hasMore);
      setLoadMoreStatus('idle');
    } catch {
        ...
        <div className="-m-1 flex items-center gap-2 overflow-x-auto p-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {genres.map((genre) => (
            <button
              ...
              className={getGenreButtonClass(activeGenre === genre.id.toString())}
            >
              {genre.name}
            </button>
          ))}
      ...
      {(canRevealMore || canRequestMore) && status !== 'loading' && showGrid && (
        <div className="mt-6 flex justify-center pb-4">
          ...
          >
            {isLoadingMore ? (
              <>
                <Icon icon="solar:spinner-bold" size={14} className="animate-spin" />
                Loading
              </>
            ) : (
              <>
                <Icon icon="solar:refresh-bold" size={14} className="transition-transform duration-300" />
