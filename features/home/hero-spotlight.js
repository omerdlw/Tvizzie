...
export default function HeroSpotlight({ items = [], onSlideChange }) {
  ...
  const heroItems = items.filter((item) => item.backdrop_path).slice(0, HERO_COUNT);
  const safeActiveIndex = heroItems.length ? Math.min(activeIndex, heroItems.length - 1) : 0;
  const current = heroItems[safeActiveIndex];

  useEffect(() => {
    setActiveIndex((previousIndex) => (heroItems.length ? Math.min(previousIndex, heroItems.length - 1) : 0));
  }, [heroItems.length]);
  ...
  const overview = current.overview?.length > 200 ? `${current.overview.slice(0, 200)}...` : current.overview;
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Image
            ...
            quality={90}
            placeholder="blur"
            blurDataURL={getImagePlaceholderDataUrl(`${current.id}-${current.backdrop_path}`)}
          />
        </motion.div>
      </AnimatePresence>

      <div className="absolute inset-0 bg-gradient-to-b from-[#e2e8f0] via-[#cbd5e1] to-[#94a3b8]" />
      <div className="absolute inset-0 bg-gradient-to-r from-[#e2e8f0] via-transparent to-transparent md:from-[#e2e8f0]" />
            ...
            <button
              ...
              className={
                index === safeActiveIndex
                  ? 'h-2 w-8 cursor-pointer bg-[#0284c7] transition-all duration-300'
                  : 'h-2 w-2 cursor-pointer bg-[#dbeafe] transition-all duration-300'
              }
            />
          ))}
        </div>
