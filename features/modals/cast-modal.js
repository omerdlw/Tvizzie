...
const GRID_CLASS = 'grid grid-cols-1 divide-y divide-black/10 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-3 lg:divide-x-0 lg:divide-y-0';
...
function createHeader({
  ...
  return {
    ...resolvedHeader,
    showClose: false,
    center: <SegmentedControl onChange={setActiveTab} items={[{
      key: 'cast',
      label: 'Cast'
    }, {
      key: 'crew',
...
export default function CastModal({
  ...
  return <ModalView close={close} contentRef={contentRef} containerClassName={containerClassName} resolvedHeader={resolvedHeader} hasCast={hasCast} hasCrew={hasCrew} hasBoth={hasBoth} castEntries={castEntries} crewEntries={crewEntries} activeEntries={activeEntries} activeTab={activeTab} />;
}
...
function ModalView({
  ...
  if (!hasCast && !hasCrew) {
    return <Container className={containerClassName} close={close} header={resolvedHeader} bodyClassName="bg-transparent p-0">
  ...
  return <Container className={containerClassName} close={close} header={resolvedHeader} bodyClassName="bg-transparent p-0">
      <div ref={contentRef} className="relative overflow-hidden">
        {!hasBoth ? <CreditsGrid close={close} list={activeEntries} keyPrefix={activeTab} /> : <>
...
function CreditsGrid({
  ...
  return <div className={GRID_CLASS}>
      {list.map((person, index) => <div key={`${keyPrefix}-${person.id || person.name || 'credit'}-${index}`} className={cn('lg:border-black/10', (index + 1) % DESKTOP_COLUMNS !== 0 && 'lg:border-r', index < lastRowStart && 'lg:border-b')}>
...
function PersonCard({
  ...
  const imageSrc = !imageError ? getPreferredPersonPosterSrc(person, 'w185') || (person.profile_path ? `${TMDB_IMG}/w185${person.profile_path}` : null) : null;
  return <Link href={`/person/${person.id}`} onClick={close} className="bg-primary/40 hover:bg-primary/60 flex items-center gap-3 p-2">
      <div className="relative h-14 w-11 shrink-0 overflow-hidden bg-black/5">
        {imageSrc ? <AdaptiveImage fill src={imageSrc} alt={person.name || 'Cast member'} sizes="44px" quality={72} className="object-cover" onError={() => setImageError(true)} wrapperClassName="h-full w-full" /> : <div className="center h-full w-full">
