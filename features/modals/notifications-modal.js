...
import { NOTIFICATION_TYPES, deleteAllNotifications, deleteNotification, markAllAsRead, markAsRead, subscribeToNotifications } from '@/core/services/notifications/notifications.service';
...
function getNotificationSubject(payload, type) {
  if (type === NOTIFICATION_TYPES.REVIEW_LIKE) {
    return payload?.subject && typeof payload.subject === 'object' ? payload.subject : {
...
export default function NotificationsModal({
  ...
  useEffect(() => {
    ...
    function projectNotifications(nextNotifications = []) {
      ...
      return (Array.isArray(nextNotifications) ? nextNotifications : []).filter(item => item?.id && !deletedIds.has(item.id)).map(item => forceReadIds.has(item.id) ? {
        ...item,
        read: true
      } : item);
    }
    ...
    resetOptimisticState();
    setIsLoading(true);
    return subscribeToNotifications(userId, nextNotifications => {
  ...
  return <ModalView close={close} header={header} isSidePosition={isSidePosition} isLoading={isLoading} notifications={notifications} hasUnread={hasUnread} unreadCount={unreadCount} handleMarkAllRead={handleMarkAllRead} handleMarkRead={handleMarkRead} handleDelete={handleDelete} handleDeleteAll={handleDeleteAll} />;
}
...
function ModalView({
  ...
  return <Container className={isSidePosition ? 'h-full max-h-full w-full sm:w-[460px]' : 'max-h-[78dvh] w-full sm:w-[min(1400px,96vw)]'} close={close} header={header} bodyClassName="p-0" footer={{
    left: <span className="text-xs opacity-70">
            {hasUnread ? `${unreadCount}unread` : `${notifications.length}notifications`}
          </span>,
    right: notifications.length > 0 ? <>
              <Button type="button" onClick={handleDeleteAll} className={cn(ACTION_BUTTON_CLASS, 'border-black/10 text-black/70 hover:bg-black/5 hover:text-black')}>
                Clear all
              </Button>

              {hasUnread && <Button type="button" onClick={handleMarkAllRead} className={cn(ACTION_BUTTON_CLASS, 'hover:bg-info hover:border-info hover:text-primary border-black bg-black text-white disabled:cursor-not-allowed disabled:border-black/5 disabled:bg-black/10 disabled:text-black/50')}>
                  Mark all as read
                </Button>}
            </> : null
  }}>
      ...
      }, (_, index) => <NotificationSkeleton key={index} />) : notifications.length === 0 ? <div className="center h-screen text-sm font-medium text-black/50">You have no notifications yet</div> : notifications.map(notification => <NotificationRow key={notification.id} notification={notification} onMarkRead={handleMarkRead} onDelete={handleDelete} />)}
      </div>
    </Container>;
}
function NotificationRow({
  ...
  return <div className={cn("grid grid-cols-[40px_minmax(0,1fr)_auto] items-center gap-3 border-b border-black/10 p-3 last:border-none lg:p-4", isUnread ? 'bg-black/5 hover:bg-black/10' : 'hover:bg-black/5')}>
      <div className="center size-10 shrink-0 overflow-hidden">
        {notification.actor ? <AdaptiveImage mode="img" src={avatarSrc} alt={notification.actor?.displayName || 'Avatar'} className="size-full object-cover" loading="lazy" decoding="async" onError={event => applyAvatarFallback(event, avatarFallbackSrc)} wrapperClassName="size-full" /> : <Icon icon={getNotificationIcon(notification.type)} size={20} className="text-black/70" />}
      </div>
      ...
      <div className="flex items-center gap-1.5">
        {isUnread && <Button onClick={event => onMarkRead(notification.id, event)} title="Mark as read" className={cn(TOOL_BUTTON_CLASS, 'border-info/10 bg-info/20 text-info hover:border-info/10 hover:bg-info/10 border')}>
            <Icon icon="material-symbols:check-rounded" size={16} />
          </Button>}

        <Button onClick={event => onDelete(notification.id, event)} title="Delete notification" variant="destructive" className={TOOL_BUTTON_CLASS}>
...
function NotificationContent({
  ...
  switch (type) {
    ...
    default:
      return <p className="text-sm text-black/70">
...
function InlineEntity({
  ...
  return href ? <Link href={href} className={className}>
      {children}
...
function InlineEntityName({
  ...
  return href ? <Link href={href} className="truncate text-sm font-semibold">
      {children}
    </Link> : <span className="truncate text-sm font-semibold">{children}</span>;
}
function NotificationSkeleton() {
  return <div className="flex items-center gap-3 border-b border-black/10 p-3 last:border-none lg:p-4">
