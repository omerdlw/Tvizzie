...
import { FOLLOW_STATUSES, acceptFollowRequest, followUser, removeFollower, rejectFollowRequest, subscribeToFollowers, subscribeToFollowing, unfollowUser } from '@/core/services/social/follows.service';
...
import { DESTRUCTIVE_ACTION_TONE_CLASS, INFO_ACTION_TONE_CLASS, SUCCESS_ACTION_TONE_CLASS } from '@/core/constants/index';
...
const ROW_BUTTON_CLASS = "h-8 w-auto shrink-0 border px-2.5 py-1 text-[11px] font-semibold disabled:cursor-not-allowed disabled:bg-black/5";
...
export default function AccountSocialModal({
  close,
  data
}) {
  const auth = useAuth();
  ...
  useEffect(() => {
    if (!userId) return setFollowersState({
      list: [],
      isLoading: false,
      error: null
    });
    setFollowersState(createCollectionState());
    return subscribeToFollowers(userId, list => setFollowersState({
      list: hydrateFollowUsers(list),
      isLoading: false,
      error: null
    }), {
      ...
      onError: error => setFollowersState({
        list: [],
        isLoading: false,
        error
      })
  ...
  useEffect(() => {
    if (!userId) return setFollowingState({
      list: [],
      isLoading: false,
      error: null
    });
    setFollowingState(createCollectionState());
    return subscribeToFollowing(userId, list => setFollowingState({
      list: hydrateFollowUsers(list),
      isLoading: false,
      error: null
    }), {
      ...
      refreshOnSubscribe: true,
      status: FOLLOW_STATUSES.ACCEPTED,
      onError: error => setFollowingState({
        list: [],
        isLoading: false,
        error
      })
  ...
  useEffect(() => {
    ...
    return subscribeToFollowers(authUserId, list => setRequestsState({
      list: hydrateFollowUsers(list),
      isLoading: false,
      error: null
    }), {
      ...
      onError: error => setRequestsState({
        list: [],
        isLoading: false,
        error
      })
  ...
  useEffect(() => {
    ...
    const sync = () => setFollowingStatusMap({
      ...acceptedStatusMap,
      ...pendingStatusMap
    });
    const unsubscribeAccepted = subscribeToFollowing(authUserId, list => {
    ...
    const unsubscribePending = subscribeToFollowing(authUserId, list => {
  ...
  const handleAccept = id => runUserAction(id, 'accept', () => acceptFollowRequest(authUserId, id), 'Request could not be accepted');
  const handleReject = id => runUserAction(id, 'reject', () => rejectFollowRequest(authUserId, id), 'Request could not be rejected');
  const handleUnfollow = id => runUserAction(id, 'unfollow', () => unfollowUser(authUserId, id), 'Could not unfollow this user');
  const handleRemoveFollower = id => runUserAction(id, 'remove-follower', () => removeFollower(authUserId, id), 'Could not remove follower');
  const handleFollow = id => runUserAction(id, 'follow', () => followUser(authUserId, id), 'Could not follow this user');
  return <ModalView close={close} activeTab={activeTab} setActiveTab={setActiveTab} tabs={tabs} list={list} isLoading={isLoading} activeErrorMessage={activeErrorMessage} emptyDescription={emptyDescription} onAccept={handleAccept} onReject={handleReject} onUnfollow={handleUnfollow} onRemoveFollower={handleRemoveFollower} onFollow={handleFollow} authUserId={authUserId} isOwnProfile={isOwnProfile} pendingActionByUserId={pendingActionByUserId} followingStatusMap={followingStatusMap} />;
}
...
function ModalView({
  ...
  return <Container className="max-h-[74vh] min-h-96 w-full sm:w-[500px]" close={close} bodyClassName="p-0" header={<SegmentedControl value={activeTab} onChange={setActiveTab} items={tabs} classNames={{
    ...
    active: 'text-white font-semibold'
      ...
      <div className="flex h-full min-h-0 flex-col">
        {isLoading ? <LoadingList /> : activeErrorMessage ? <EmptyState description={activeErrorMessage} className="h-full" /> : list.length === 0 ? <EmptyState description={emptyDescription} className="h-full min-h-96" /> : <div className="min-h-96 flex-1 overflow-y-auto">
            {list.map(user => <SocialUserRow key={user.id} close={close} user={user} action={<UserAction tab={activeTab} user={user} authUserId={authUserId} isOwnProfile={isOwnProfile} pendingKind={pendingActionByUserId[user.id] || null} followStatus={followingStatusMap[user.id] || null} actions={actions} />} />)}
          </div>}
      </div>
    </Container>;
}
function SocialUserRow({
  ...
  return <div className="flex items-center justify-between gap-3 border-b border-black/10 p-3 last:border-none hover:bg-black/5 lg:p-4">
      <Link href={`/account/${user.username || user.id}`} onClick={close} className="flex min-w-0 flex-1 items-center gap-2.5">
        <AdaptiveImage mode="img" src={avatarSrc} alt={user.displayName} loading="lazy" decoding="async" className="size-10 shrink-0 object-cover" onError={event => applyAvatarFallback(event, avatarFallbackSrc)} wrapperClassName="size-10 shrink-0" />
...
function UserAction({
  ...
  const canShowFollowAction = tab !== TABS.INBOX && Boolean(authUserId) && !isOwnProfile && authUserId !== user.id && followStatus !== FOLLOW_STATUSES.ACCEPTED;
  if (tab === TABS.INBOX) {
  ...
  if (tab === TABS.FOLLOWING && isOwnProfile) {
    return <Button onClick={() => onUnfollow(user.id)} disabled={isPending} className={ACTION_CLASSES.ERROR}>
        {pendingKind === 'unfollow' ? 'Unfollowing' : 'Unfollow'}
      </Button>;
  }
  if (tab === TABS.FOLLOWERS && isOwnProfile) {
    return <Button onClick={() => onRemoveFollower(user.id)} disabled={isPending} className={ACTION_CLASSES.ERROR}>
  ...
  if (canShowFollowAction) {
    ...
    return <Button onClick={() => onFollow(user.id)} disabled={isFollowPending || isPending} className={ACTION_CLASSES.INFO}>
        {followLabel}
      </Button>;
  }
  return null;
...
function LoadingList() {
  return <div>
      {Array.from({
      length: 10
    }, (_, index) => <div key={index} className="flex items-center gap-3 border-b border-black/10 p-3 last:border-none lg:p-4">
