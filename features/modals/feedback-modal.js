...
export default function FeedbackModal({
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const didStore = setStorageItem(FEEDBACK_STORAGE_KEY, [nextRecord, ...safeEntries].slice(0, FEEDBACK_STORAGE_LIMIT));
      const copied = didStore ? await copyFeedbackRecord(nextRecord) : false;
      toast.error(didStore ? copied ? 'Feedback could not be sent. Draft saved locally and copied.' : 'Feedback could not be sent. Draft saved locally.' : error?.message || 'Feedback could not be sent');
    } finally {
      setIsSaving(false);
    }
  }
  return <ModalView close={close} header={header} message={message} setMessage={setMessage} isSaving={isSaving} handleSubmit={handleSubmit} />;
}
...
function ModalView({
  ...
  return <Container className="w-full sm:w-[580px]" header={header} close={close} bodyClassName="p-4" footer={{
    right: <>
            <Button type="button" onClick={close} className={cn(ACTION_BUTTON_CLASS, 'border-black/10 text-black/70 hover:bg-black/5 hover:text-black')}>
              Cancel
            </Button>
            <Button type="submit" form={FORM_ID} disabled={isSaving} className={cn(ACTION_BUTTON_CLASS, 'hover:bg-info hover:border-info hover:text-primary border-black bg-black text-white disabled:cursor-not-allowed disabled:border-black/5 disabled:bg-black/10 disabled:text-black/50')}>
      ...
      <form id={FORM_ID} onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="space-y-2">
          <Textarea value={message} onChange={event => setMessage(event.target.value)} maxHeight={220} placeholder="Your message" className={{
          wrapper: "focus-within:bg-primary border border-black/10 bg-white focus-within:border-black/15 hover:border-black/15",
          textarea: 'min-h-[160px] w-full bg-transparent px-4 py-3 text-sm text-black outline-none placeholder:text-black/50'
        }} />
        </div>
      </form>
    </Container>;
