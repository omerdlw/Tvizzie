...
import { getReviewMinLength, getReviewValidationError, upsertListReview, upsertMediaReview } from '@/core/services/media/reviews';
...
const SECONDARY_BUTTON_CLASS = "h-8 shrink-0 border border-black/10 px-4 text-xs font-semibold tracking-wide uppercase text-black/70 hover:bg-black/5 hover:text-black";
const PRIMARY_BUTTON_CLASS = "h-8 border border-black bg-black px-4 text-xs font-semibold tracking-wide uppercase text-white hover:border-info hover:bg-info hover:text-primary disabled:cursor-not-allowed disabled:border-black/5 disabled:bg-black/10 disabled:text-black/50";
...
function resolveListContext(data = {}, review = null) {
  ...
  const ownerId = data?.ownerId || data?.list?.ownerId || data?.list?.ownerSnapshot?.id || review?.subjectOwnerId || null;
  ...
  return {
    ...
    list: {
      ...
      previewItems: Array.isArray(data?.list?.previewItems) ? data.list.previewItems : Array.isArray(review?.subjectPreviewItems) ? review.subjectPreviewItems : [],
      ownerSnapshot: {
        id: ownerId,
...
function buildUpdatedReview({
  ...
  const nextSubject = {
    ...
    subjectPreviewItems: Array.isArray(savedSubject.subjectPreviewItems) ? savedSubject.subjectPreviewItems : Array.isArray(review?.subjectPreviewItems) ? review.subjectPreviewItems : [],
...
export default function ReviewEditorModal({
  ...
  async function handleSubmit(event) {
    ...
    setIsSaving(true);
    try {
      const savedSubject = subjectContext.subjectType === 'list' ? await upsertListReview({
        content,
        isSpoiler: spoiler,
  ...
  return <ModalView close={close} isListSubject={isListSubject} rating={rating} setRating={setRating} hasText={hasText} trimmedTextLength={trimmedTextLength} validationError={validationError} isSaving={isSaving} hasExistingReview={hasExistingReview} reviewText={reviewText} modalSubjectTitle={modalSubjectTitle} isSpoiler={isSpoiler} handleSubmit={handleSubmit} handleTextChange={handleTextChange} handleSpoilerToggle={handleSpoilerToggle} />;
}
...
function ModalView({
  ...
  return <Container className="w-full sm:w-[640px]" header={isListSubject ? null : <RatingSelector value={rating} onChange={setRating} />} close={close} footer={{
    left: <FooterMeta hasText={hasText} isList={isListSubject} trimmedTextLength={trimmedTextLength} validationError={validationError} />,
    right: <>
            <Button type="button" onClick={close} className={SECONDARY_BUTTON_CLASS}>
              Cancel
            </Button>
            <Button type="submit" form={FORM_ID} disabled={isSaving || Boolean(validationError)} className={PRIMARY_BUTTON_CLASS}>
              {isSaving ? 'Saving' : getPrimaryActionLabel({
          hasExistingReview,
      ...
      <form id={FORM_ID} onSubmit={handleSubmit}>
        <Textarea maxLength={800} value={reviewText} placeholder={isListSubject ? `Share your thoughts on ${modalSubjectTitle}` : `Add your thoughts about ${modalSubjectTitle} (optional)`} onChange={handleTextChange} className={{
        wrapper: 'flex',
        textarea: 'min-h-[180px] w-full resize-none p-3 text-sm leading-normal outline-none placeholder:text-black/50'
      }} />
        <SpoilerToggle disabled={!hasText} checked={isSpoiler} invalid={Boolean(validationError)} onClick={handleSpoilerToggle} />
      </form>
    </Container>;
}
function FooterMeta({
  ...
  return <div>
      <div className="text-xs text-black/70">
        {hasText ? `${trimmedTextLength} characters` : isList ? `Comment required (${REVIEW_MIN_LENGTH}+ characters)` : `Optional text (${REVIEW_MIN_LENGTH}+ if added)`}
      </div>
...
function SpoilerToggle({
  ...
  return <button type="button" role="switch" aria-checked={!disabled && checked} disabled={disabled} onClick={onClick} className={cn("flex w-full items-center justify-between border-t p-4 text-left", disabled && 'cursor-not-allowed border-black/10 text-black/50', !disabled && checked && 'bg-error/10 text-error hover:bg-error/20 border-black/10', !disabled && !checked && 'bg-primary border-black/10 hover:bg-black/5', invalid && 'border-t')}>
      <div>
      ...
      <span className={cn("relative inline-flex h-6 w-11 shrink-0 items-center border p-px", checked && !disabled ? 'border-error bg-error' : 'border-black/5 bg-black/5')}>
        <span className={cn("bg-primary size-5", checked && !disabled ? 'bg-primary translate-x-5' : 'translate-x-0')} />
      </span>
    </button>;
}
