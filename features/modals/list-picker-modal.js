...
export default function ListPickerModal({
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    const unsubscribe = subscribeToUserLists(userId, nextLists => {
  ...
  return <ModalView close={close} lists={lists} draftMemberships={draftMemberships} isLoading={isLoading} isApplying={isApplying} selectedCount={selectedCount} pendingChangesCount={pendingChangesCount} hasPendingChanges={hasPendingChanges} handleOpenCreator={handleOpenCreator} handleToggleDraft={handleToggleDraft} handleApplyChanges={handleApplyChanges} />;
}
...
function ModalView({
  ...
  return <Container className="max-h-[72dvh] w-full sm:w-[660px]" header={false} close={close} bodyClassName="p-4" footer={{
    left: <span className="text-xs text-black/70">
            {selectedCount} selected • {pendingChangesCount} pending
          </span>,
    right: <>
            <Button type="button" onClick={close} disabled={isApplying} className={`${ACTION_BUTTON_CLASS}text-black/70 hover:bg-black/5 hover:text-black`}>
              Cancel
            </Button>
            <Button type="button" onClick={handleApplyChanges} disabled={isApplying || !hasPendingChanges} className="hover:bg-info hover:border-info hover:text-primary h-8 border border-black bg-black px-4 text-xs font-semibold tracking-wide text-white uppercase disabled:cursor-not-allowed disabled:border-black/5 disabled:bg-black/10 disabled:text-black/50">
      ...
      <section className="flex min-h-0 flex-col gap-3">
        <header className="mb-1 flex items-center justify-between gap-3 px-1">
          ...
          <Button type="button" onClick={handleOpenCreator} disabled={isApplying} className={`${ACTION_BUTTON_CLASS}text-black/70 hover:bg-black/5 hover:text-black`}>
            Create new list
          </Button>
        </header>
        ...
        <div className="max-h-[56dvh] min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain pr-1">
          ...
          {!isLoading && lists.length === 0 && <div className="flex min-h-40 flex-col items-center justify-center border border-dashed border-black/15 bg-black/2 text-center">
          ...
          return <ListRow key={list.id} list={list} isSelected={isSelected} onToggle={() => handleToggleDraft(list.id)} />;
        })}
        </div>
      </section>
    </Container>;
...
function ListRow({
  ...
  return <button type="button" onClick={onToggle} className={cn("group flex w-full items-center gap-4 border p-3 text-left", isSelected ? 'bg-info/20 border-black/20' : 'hover:bg-primary border-black/10 hover:border-black/15')}>
      ...
      <span className={cn("flex size-[22px] shrink-0 items-center justify-center border", isSelected ? 'border-info bg-info text-primary' : 'border-black/10 text-black/50 group-hover:border-black/40 group-hover:text-black/70')}>
...
function ListPreviewStack({
  ...
  return <div className="relative h-[68px] w-[82px] shrink-0">
      {previewItems.length === 0 ? <div className="center absolute bottom-0 left-0 h-[68px] w-[46px] border border-dashed border-black/10 bg-white text-black/50">
          <Icon icon="solar:list-bold" size={20} />
        </div> : previewItems.map((item, index) => {
      const imageSrc = getPreviewImage(item);
      return <div key={item.mediaKey || `${item.entityType}-${item.entityId}-${index}`} className="border-primary absolute bottom-0 overflow-hidden border-[1.5px] bg-white shadow-xs ring-1 ring-black/5" style={{
        width: '46px',
        height: `${68 - index * 6}px`,
        left: `${index * 12}px`,
        zIndex: previewItems.length - index
      }}>
              {imageSrc ? <AdaptiveImage mode="img" src={imageSrc} alt={item.title || item.name || 'Poster'} loading="lazy" decoding="async" className="h-full w-full object-cover" wrapperClassName="h-full w-full" /> : <div className="center bg-primary h-full w-full text-black/50">
...
function LoadingSkeleton() {
  return <div className="space-y-2.5">
      {Array.from({
      length: 10
    }).map((_, index) => <div key={`list-picker-skeleton-${index}`} className="flex h-24 items-center gap-4 border border-black/10 bg-white/80 p-3">
          <div className="relative h-[68px] w-[82px] shrink-0">
            {[0, 1, 2, 3].map(stackIndex => <div key={`list-picker-skeleton-stack-${index}-${stackIndex}`} className="absolute bottom-0 overflow-hidden border border-black/[0.08] shadow-[0_1px_2px_rgba(0,0,0,0.06)]" style={{
          backgroundColor: LIST_PICKER_STACK_SKELETON_BACKGROUNDS[stackIndex] || LIST_PICKER_STACK_SKELETON_BACKGROUNDS[LIST_PICKER_STACK_SKELETON_BACKGROUNDS.length - 1],
          width: '46px',
