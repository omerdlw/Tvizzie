...
export default function ReviewAuthFallback({ mode = 'review', onSignIn, title }) {
  ...
  return (
    <div className="grid-diamonds-top w-full border-t border-white/10">
      <div className="movie-detail-shell-inset flex w-full flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between sm:py-5">
        ...
        <Button
          type="button"
          className="bg-primary/40 inline-flex min-h-10 w-full items-center justify-center gap-2 border border-white/5 px-4 py-2 text-[11px] font-semibold tracking-wide text-white/70 uppercase hover:bg-white hover:text-black sm:w-auto"
          onClick={onSignIn}
        >
          Sign In
        </Button>
