...
export default function ReviewList({
  ...
  return <div className="flex flex-col">
      ...
      return <div key={review.docPath || review.id || `review-${index}`} style={{
        willChange: 'transform, opacity, filter'
      }}>
            <ReviewCard className={sortedReviews[0] === review ? 'pt-0 pb-6' : ''} review={mergedReview} currentUserId={currentUserId} displayVariant={displayVariant} isOwnReview={showOwnActions && isOwnReview} likedMediaKeys={likedMediaKeys} onLike={() => onLike(review)} onEdit={() => onEdit(review)} onDeleteRequest={() => onDeleteRequest(review)} rewatchMediaKeys={rewatchMediaKeys} showSubject={showSubject} watchedMediaKeys={watchedMediaKeys} />
          </div>;
    })}
    </div>;
}
