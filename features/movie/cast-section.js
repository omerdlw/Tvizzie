...
function PersonImage({
  ...
  if (!src) {
    return <div className="center h-full w-full rounded">
        <Icon icon="solar:user-bold" size={size === 'w92' ? 14 : 20} className="text-black/50" />
      </div>;
  }
  return <AdaptiveImage fill alt={person.name} src={src} sizes={size === 'w92' ? '32px' : '64px'} priority={priority} fetchPriority={resolveImageFetchPriority({
    fetchPriority,
    priority
  })} loading={resolveImageLoading({
    priority
  })} quality={resolveImageQuality('thumbnail', quality)} decoding="async" draggable={false} className="object-cover" onError={() => setError(true)} wrapperClassName="h-full w-full" />;
}
function PersonCard({
  person,
  compact = false,
  ...
  return <Link href={`/person/${person.id}`} onDragStart={e => e.preventDefault()} className={["group bg-primary/30 hover:bg-primary/60 flex items-center gap-3 border border-black/10 backdrop-blur-xs hover:border-black/15", compact ? 'h-10 min-w-0 flex-1 p-1 pr-2' : 'p-1 pr-4'].join(' ')}>
      <div className={['relative shrink-0 overflow-hidden', compact ? 'h-8 w-8' : 'h-20 w-16'].join(' ')}>
        <PersonImage person={person} size={compact ? 'w92' : 'w185'} quality={compact ? 70 : 72} priority={priority} fetchPriority={fetchPriority} />
      </div>

      {compact ? <span className="truncate text-xs font-semibold text-black">
          {person.name}
        </span> : <div className="flex min-w-0 flex-col">
...
function buildPersonEntryKey(tabKey, person = {}, index = 0, variant = 'entry') {
  const creditKey = person?.credit_id || person?.creditId || person?.cast_id || person?.castId || person?.order || [person?.id, person?.job, person?.department, person?.character, person?.subtitle].filter(Boolean).join('-') || person?.name || 'person';
...
export default function CastSection({
  ...
  const crewEntries = useMemo(() => crew.map(item => ({
    ...item,
    subtitle: item?.job || item?.department || 'Crew'
  })), [crew]);
  const tabs = useMemo(() => {
    const items = [];
    if (castEntries.length) items.push({
      key: 'cast',
      label: 'Cast',
      entries: castEntries
    });
    if (crewEntries.length) items.push({
      key: 'crew',
      label: 'Crew',
      entries: crewEntries
    });
  ...
  const renderPanel = (tabKey, entries) => {
    ...
    return <div className="flex flex-col gap-2">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          return <div key={buildPersonEntryKey(tabKey, person, index, 'featured')}>
                <PersonCard person={person} priority={index < 4} fetchPriority={index < 4 ? 'high' : undefined} />
              </div>;
        })}
        </div>

        {!!compact.length && <div className="flex h-10 items-center gap-2">
          ...
          return <div key={buildPersonEntryKey(tabKey, person, index, 'compact')} className={`min-w-0 flex-1 ${responsiveClass}`}>
                  <PersonCard person={person} compact />
                </div>;
        })}

            <button type="button" aria-label="Show full cast" onClick={handleOpenModal} className="center bg-primary/30 hover:bg-primary/60 size-10 shrink-0 border border-black/10 text-black/70 hover:border-black/15 hover:text-black">
  ...
  return <MovieSurfaceReveal>
      <section className="relative flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          ...
          indicator: ''
        }} onChange={setActiveTab} items={tabs.map(({
          key,
          label
        }) => ({
          key,
