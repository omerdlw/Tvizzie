...
export default function VideosSection({
  ...
  const items = useMemo(() => availableTypes.map(type => ({
    key: type,
  ...
  return <MovieSurfaceReveal>
      <section className="flex w-full flex-col gap-3">
        ...
        button: '',
        indicator: ''
      }} items={items} onChange={setActiveType} />
            ...
            <div key={`movie-videos-${activeType || 'all'}`}>
              <Carousel gap="gap-3">
                ...
                return <div key={video.id}>
                      <MediaCard className="w-72" aspectClass="aspect-video" imageSrc={`https://img.youtube.com/vi/${video.key}/hqdefault.jpg`} imageAlt={video.name} imageSizes="288px" imagePreset="grid" fallbackIcon="solar:video-library-bold" fallbackIconSize={24} overlay={<>
                          ...
                          </>} onClick={() => openModal('VIDEO_PREVIEW_MODAL', 'center', {
                    data: video
                  })} />
                    </div>;
              })}
