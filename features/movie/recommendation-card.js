...
import { getSurfaceItemMotion, useInitialItemRevealEnabled } from '@/features/media/static-route-elements';
...
export default function RecommendationCard({
  ...
  return <div>
      <MediaCard imageSrc={(mediaType === 'movie' ? getPreferredMoviePosterSrc(movie, 'w342') : null) || (movie.poster_path ? `${TMDB_IMG}/w342${movie.poster_path}` : null)} imageFetchPriority={imageFetchPriority} imagePriority={imagePriority} imagePreset="poster" href={getMediaDetailPath({
      entityId: movie.id,
      entityType: mediaType
    })} tooltipText={tooltipText} imageAlt={resolvedTitle} className="w-full" />
    </div>;
}
