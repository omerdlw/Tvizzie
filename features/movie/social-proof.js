...
function getSummaryParts({
  ...
  return [likes.count > 0 && `${likes.count} likes`, watchlist.count > 0 && `${watchlist.count} watchlist`, reviews.count > 0 && `${reviews.count} reviews`].filter(Boolean);
}
export default function MediaSocialProof({
  media,
  viewerId
}) {
  const auth = useAuth();
  ...
  const handleOpenModal = () => {
    openModal('MEDIA_SOCIAL_PROOF_MODAL', {
      desktop: 'right',
      mobile: 'right'
    }, {
      header: {
  ...
  return <button type="button" aria-label="Open social activity" onClick={handleOpenModal} className="group inline-flex items-center gap-1 px-2 py-1 text-[11px] font-semibold tracking-widest text-black/70 uppercase hover:text-black">
