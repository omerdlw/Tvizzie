...
const ACCOUNT_HERO_BANNER_WRAPPER_CLASS = 'mx-auto h-full w-full sm:w-[88%] lg:w-[70%] [mask-image:none] [-webkit-mask-image:none] sm:[mask-image:linear-gradient(90deg,transparent_0%,black_8%,black_92%,transparent_100%)] sm:[-webkit-mask-image:linear-gradient(90deg,transparent_0%,black_8%,black_92%,transparent_100%)]';
...
const ACCOUNT_HERO_RIGHT_FADE_CLASS = 'account-hero-right-fade absolute inset-y-0 right-0 w-[16%] sm:w-[26%] lg:w-[34%]';
...
const ACCOUNT_HERO_CENTER_GLOW_CLASS = 'absolute left-1/2 top-[16%] h-40 w-40 -translate-x-1/2 bg-white/60 blur-3xl sm:h-64 sm:w-64';
...
function HeroInlineMetric({
  ...
  if (item.href) {
    return <Link href={item.href} className={wrapperClassName}>
        {content}
      </Link>;
  }
  if (typeof item.onClick === 'function') {
    return <button type="button" onClick={item.onClick} className={cn('border-0 bg-transparent p-0 text-left', wrapperClassName)}>
        {content}
      </button>;
  }
  return <span className={wrapperClassName}>{content}</span>;
}
function HeroBioPreview({
  description,
  onReadMore
}) {
  ...
  return <div className="relative mt-2 w-full">
      <p ref={textRef} className={cn('line-clamp-2 text-sm leading-6 break-words')}>
        {description}
      </p>
      {shouldShowReadMore ? <div className="account-hero-text-fade absolute right-0 bottom-0 flex h-[24px] items-center justify-end pl-12">
  ...
  return <div className="w-full min-w-0 text-left">
      ...
      <div className="mt-2 flex flex-col gap-0.5 text-sm">
        <div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-1 min-[420px]:grid-cols-3 lg:hidden">
          {mobileStats.map((item, index) => <HeroInlineMetric key={`${item.label}-${item.value}-${index}`} item={item} className="inline-flex min-w-0 flex-col items-start gap-0.5 text-left" valueClassName="text-base font-semibold leading-none tracking-tight" labelClassName="max-w-full truncate text-[13px] leading-none text-black/75" />)}
        </div>

        <div className="hidden items-center gap-x-6 gap-y-2 lg:flex lg:gap-x-7">
          {countsLabel.map((item, index) => <HeroInlineMetric key={`${item.label}-${item.value}-${index}`} item={item} className="inline-flex items-baseline gap-1.5 whitespace-nowrap" valueClassName="text-base font-semibold leading-none tracking-tight " labelClassName="text-base leading-none text-black/75" />)}
        </div>
      </div>
    </div>;
}
function HeroStatsGrid({
  ...
  return <div className={className}>
      {stats.map((stat, index) => stat.href ? <Link key={`${stat.label}-${stat.value}-${index}`} href={stat.href} className={cn(itemClassName, "")}>
            <div className={valueClassName}>{formatHeroCount(stat.value)}</div>
            <div className={labelClassName}>{stat.label}</div>
          </Link> : typeof stat.onClick === 'function' ? <button key={`${stat.label}-${stat.value}-${index}`} type="button" onClick={stat.onClick} className={cn('border-0 bg-transparent p-0', itemClassName, "")}>
            <div className={valueClassName}>{formatHeroCount(stat.value)}</div>
            <div className={labelClassName}>{stat.label}</div>
          </button> : <div key={`${stat.label}-${stat.value}-${index}`} className={itemClassName}>
            <div className={valueClassName}>{formatHeroCount(stat.value)}</div>
            <div className={labelClassName}>{stat.label}</div>
          </div>)}
    </div>;
...
export default function AccountHero({
  ...
  const heroBannerSrc = resolveVersionedImageUrl(String(profile?.bannerUrl || '')).trim().replace(/^(null|undefined)$/i, '') || null;
  const resolvedWatchedCount = watchedCount !== null && watchedCount !== undefined && Number.isFinite(Number(watchedCount)) ? Number(watchedCount) : Number(profile?.watchedCount || 0);
  const heroCountItems = [createHeroCollectionMetaItem(watchlistCount, 'Watchlist', 'Watchlist', {
  ...
  return <section className={`relative w-full overflow-hidden border-b border-black/15 ${ACCOUNT_HERO_HEIGHT_CLASS}`}>
      {heroBannerSrc ? <div className="absolute inset-0">
          <AdaptiveImage mode="img" src={heroBannerSrc} className={ACCOUNT_HERO_IMAGE_CLASS} alt={`${heroDisplayName} cover`} loading="eager" fetchPriority="high" decoding="async" wrapperClassName={ACCOUNT_HERO_BANNER_WRAPPER_CLASS} />
        </div> : null}
      ...
      <div className={`${ACCOUNT_ROUTE_SHELL_CLASS} relative flex ${ACCOUNT_HERO_HEIGHT_CLASS} items-end px-4 pt-18 pb-5 sm:px-8 sm:pt-24 sm:pb-7 lg:pb-8`}>
        <div className="flex w-full flex-col gap-2 sm:gap-3">
          <div className="grid w-full gap-y-4 lg:grid-cols-[128px_minmax(0,1fr)_280px] lg:grid-rows-[auto_auto] lg:items-end lg:gap-x-8 lg:gap-y-0">
            <div className="h-24 w-24 justify-self-start overflow-hidden sm:h-32 sm:w-32 lg:row-span-2 lg:self-end">
              <AdaptiveImage mode="img" className="h-full w-full object-cover" src={heroAvatarSrc} alt={heroDisplayName} decoding="async" onError={event => applyAvatarFallback(event, heroAvatarFallbackSrc)} wrapperClassName="h-full w-full" />
            </div>

            <div className="lg:col-start-2 lg:row-span-2 lg:self-end">
              <HeroTextContent countsLabel={heroCountItems} displayName={heroDisplayName} mobileStats={mobileHeroStats} />
            </div>

            <div className="hidden lg:col-start-3 lg:row-span-2 lg:block lg:self-center lg:justify-self-end">
              <HeroStatsGrid stats={heroStats} className="grid grid-cols-2 gap-6 text-center" itemClassName="inline-flex min-w-0 items-baseline gap-1.5 py-1 whitespace-nowrap lg:flex-col lg:items-center lg:gap-0.5 lg:py-0" valueClassName="text-base font-semibold leading-none tracking-tight sm:text-lg lg:text-[2.4rem]" labelClassName="text-base leading-none text-black/75 lg:mt-1 lg:text-[9px] lg:uppercase lg:tracking-widest" />
            </div>
          </div>
