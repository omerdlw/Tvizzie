...
const ACCOUNT_HERO_HEIGHT_CLASS =
  'min-h-[500px] sm:min-h-[620px] lg:min-h-[600px]'
...
const ACCOUNT_HERO_AMBIENT_OVERLAY_CLASS =
  'absolute inset-0 bg-[linear-gradient(180deg,rgba(8,14,20,0.18)_0%,rgba(8,14,20,0.14)_18%,rgba(8,14,20,0.1)_34%,rgba(4,7,10,0.24)_56%,rgba(0,0,0,0.62)_74%,rgba(0,0,0,0.88)_88%,rgba(0,0,0,0.98)_96%,rgb(0,0,0)_100%)]'
const ACCOUNT_HERO_LEFT_FADE_CLASS =
  'absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,0.97)_0%,rgba(0,0,0,0.94)_12%,rgba(0,0,0,0.82)_24%,rgba(0,0,0,0.6)_36%,rgba(0,0,0,0.34)_50%,rgba(0,0,0,0.12)_62%,rgba(0,0,0,0.03)_70%,rgba(0,0,0,0)_76%)]'
const ACCOUNT_HERO_RIGHT_FADE_CLASS =
  'absolute inset-0 hidden bg-[linear-gradient(270deg,rgba(0,0,0,0.97)_0%,rgba(0,0,0,0.94)_12%,rgba(0,0,0,0.82)_24%,rgba(0,0,0,0.6)_36%,rgba(0,0,0,0.34)_50%,rgba(0,0,0,0.12)_62%,rgba(0,0,0,0.03)_70%,rgba(0,0,0,0)_76%)] lg:block'
const ACCOUNT_HERO_TOP_FADE_CLASS =
  'absolute inset-x-0 top-0 h-32 bg-[linear-gradient(180deg,rgba(0,0,0,0.34)_0%,rgba(0,0,0,0.2)_38%,rgba(0,0,0,0.08)_68%,rgba(0,0,0,0)_100%)] sm:h-36'
...
const ACCOUNT_HERO_CENTER_GLOW_CLASS =
  'absolute left-1/2 top-[16%] h-40 w-40 -translate-x-1/2 rounded-full bg-white/10 blur-3xl sm:h-64 sm:w-64'

function formatHeroCount(value) {
  return new Intl.NumberFormat('en-US').format(Number(value) || 0)
}

function createHeroCollectionMetaItem(
  count,
  singular,
  plural = `${singular}s`,
  options = {}
...
function HeroInlineMetric({
  ...
  const wrapperClassName = cn(
    className,
    (item.href || typeof item.onClick === 'function') &&
...
function HeroBioPreview({ description, onReadMore }) {
  ...
  useEffect(() => {
    ...
    if (!container || !measureElement || !description) {
    ...
    const updatePreview = () => {
      const containerWidth = Math.max(container.clientWidth || 0, 1)
      ...
      const setMeasureContent = (value, withSuffix = false) => {
        measureElement.textContent = withSuffix ? `${value}... Read more` : value
      }

      setMeasureContent(description)
      ...
      const wordBoundaryIndex = bestFit.lastIndexOf(' ')
      const normalizedPreview =
        wordBoundaryIndex > Math.floor(bestFit.length * 0.6)
          ? bestFit.slice(0, wordBoundaryIndex).trimEnd()
          : bestFit

      setPreviewText(normalizedPreview || description.slice(0, 1))
  ...
  return (
    <div ref={containerRef} className="relative mt-2.5 w-full max-w-[560px] sm:mt-3">
      <p className="wrap-break-word text-sm leading-6 text-white">
        {isOverflowing ? `${previewText}...` : previewText}
        {isOverflowing && typeof onReadMore === 'function' ? (
          <>
            ...
            <button
              type="button"
              onClick={onReadMore}
              className="inline whitespace-nowrap text-sm font-medium text-white transition hover:text-white"
            >
              Read more
            </button>
          </>
      ...
      <p
        ref={measureRef}
        aria-hidden="true"
        className="pointer-events-none invisible absolute -z-10 max-w-full wrap-break-words text-sm leading-6"
      />
    </div>
  )
}
  ...
  return (
    <div className="w-full max-w-[560px] text-left">
      <div className="flex items-center gap-1">
        <h1 className="min-w-0 text-[2rem] font-bold leading-none tracking-tight sm:text-[2.25rem] lg:text-[3.35rem]">
          {displayName}
        </h1>
      </div>
        ...
        <div className="grid grid-cols-3 gap-x-5 gap-y-4 pt-1 lg:hidden">
          {mobileStats.map((item, index) => (
            <HeroInlineMetric
              ...
              item={item}
              className="inline-flex min-w-0 items-baseline gap-1.5 whitespace-nowrap text-left"
              valueClassName="text-base font-semibold leading-none tracking-tight text-white"
              labelClassName="truncate text-base leading-none text-white"
            />
          ))}
        </div>
        ...
        <div className="hidden flex-wrap items-center gap-x-6 gap-y-2 lg:flex lg:gap-x-7">
          {countsLabel.map((item, index) => (
            <HeroInlineMetric
              ...
              valueClassName="text-base font-semibold leading-none tracking-tight text-white"
              labelClassName="text-base leading-none text-white"
            />
          ))}
        </div>
      </div>
      ...
      {stats.map((stat, index) => (
        stat.href ? (
          <Link
            ...
            href={stat.href}
            className={cn(itemClassName, 'transition-opacity hover:opacity-70')}
          >
...
export default function AccountHero({
  ...
  const resolvedWatchedCount =
    ...
    Number.isFinite(Number(watchedCount))
    ? Number(watchedCount)
  ...
  return (
    <section
      className={`relative w-full overflow-hidden border-b border-white/10 text-white ${ACCOUNT_HERO_HEIGHT_CLASS}`}
    >
      ...
      <div
        className={`${ACCOUNT_ROUTE_SHELL_CLASS} relative flex ${ACCOUNT_HERO_HEIGHT_CLASS} items-end px-4 pb-8 pt-20 sm:px-8 sm:pb-11 sm:pt-24 lg:pb-12`}
      >
        <div className="grid w-full gap-y-4 sm:gap-y-5 lg:grid-cols-[128px_minmax(0,1fr)_280px] lg:items-end lg:gap-x-8 lg:gap-y-0">
          ...
          <div className="hidden lg:col-start-3 lg:row-start-1 lg:block lg:justify-self-end">
            <HeroStatsGrid
              ...
              valueClassName="text-base font-semibold leading-none tracking-tight text-white sm:text-lg lg:text-[2.4rem]"
              labelClassName="text-base leading-none text-white lg:mt-1 lg:text-[9px] lg:uppercase lg:tracking-widest"
            />
          </div>
        </div>
      </div>
