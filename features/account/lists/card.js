...
function PreviewPoster({
  ...
  const {
    ...
    y,
    zIndex
  } = getPosterMetrics(index, total, isHovered);
  return <div className="absolute top-0 left-1/2" style={{
    zIndex
  }}>
      ...
      width: `${POSTER_WIDTH}px`
        ...
        {imageSrc ? <img src={imageSrc} alt={item.title || item.name || 'Poster'} className="h-full w-full object-cover" /> : <div className="center h-full w-full bg-white/50 text-black/50">
...
function PlaceholderPoster({
  ...
  return <div className="absolute top-0 left-1/2" style={{
    zIndex
      ...
      <div className="border border-dashed border-black/10 bg-white" style={{
      height: `${POSTER_HEIGHT}px`,
      width: `${POSTER_WIDTH}px`
    }} />
    </div>;
...
export default function AccountListCard({
  ...
  const previewItems = useMemo(() => Array.isArray(list?.previewItems) ? list.previewItems.slice(0, STACK_SIZE) : [], [list?.previewItems, posterPreferenceVersion]);
  ...
  const itemsCount = Number.isFinite(Number(list?.itemsCount)) ? Number(list.itemsCount) : Array.isArray(list?.previewItems) ? list.previewItems.length : 0;
  const likesCount = Number.isFinite(Number(list?.likesCount)) ? Number(list.likesCount) : Array.isArray(list?.likes) ? list.likes.length : 0;
  const reviewsCount = Number.isFinite(Number(list?.reviewsCount)) ? Number(list.reviewsCount) : 0;
  return <article className="relative w-full" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <Link href={getListHref(list, ownerUsername)} className="block">
        <div className="relative w-full" style={{
        perspective: '1200px'
      }}>
          <div className="relative z-0 border border-black/10 bg-white" style={{
          ...
          transformStyle: 'preserve-3d'
            ...
            transformStyle: 'flat'
              ...
              {previewSlots.map((item, index) => item ? <PreviewPoster key={item.mediaKey || `${item.entityType}-${item.entityId}-${index}`} index={index} isHovered={isHovered} item={item} total={previewSlots.length} /> : <PlaceholderPoster key={`placeholder-${index}`} index={index} isHovered={isHovered} total={previewSlots.length} />)}
            </div>
          </div>

          <div className="absolute right-0 bottom-0 left-0 z-10 overflow-hidden border border-black/10 bg-white/80" style={{
            ...
            <div className="relative h-11 border-t border-black/10">
              <div className="absolute inset-0 flex items-center justify-between px-3 text-[13px] text-black/70">
                ...
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {typeof renderActions === 'function' ? <div className="shrink-0" onClick={event => {
