...
export default function AccountFavoritesOverview({
  ...
  return <AccountSectionLayout icon={icon} showSeeMore={showSeeMore} summaryLabel={summaryLabel} title={title} titleHref={titleHref}>
      {cards.length > 0 ? <div className="grid w-full grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-3 md:grid-cols-5">
          {cards.slice(0, OVERVIEW_ROW_CARD_LIMIT).map((card, index) => <div key={`${card.id}-${index}`} className="flex h-full min-w-0 flex-col">
              <MediaCard className="w-full md:w-full lg:w-full" href={card.href} imageAlt={card.imageAlt} imageSizes="(max-width: 767px) 33vw, (max-width: 1023px) 25vw, 20vw" imageSrc={card.imageSrc} topOverlay={typeof renderOverlay === 'function' ? renderOverlay(card.item) : isOwner && typeof onRemoveItem === 'function' ? <div className="absolute inset-x-0 top-0 flex justify-end p-2">
                      <Button aria-label={`Remove${card.imageAlt}from favorites showcase`} variant="destructive-icon" className={'text-error hover:border-error hover:bg-error border border-black/15 bg-white hover:text-white'} disabled={pendingItemId === card.id} onClick={async event => {
            ...
            try {
              ...
              setPendingItemId(currentId => currentId === card.id ? null : currentId);
                        ...
                        <Icon className={pendingItemId === card.id ? "" : ''} icon="solar:trash-bin-trash-bold" size={16} />
                      </Button>
