  ...
  return (
    ...
    >
      {listedReviewCount === 0 && !isLoading && !loadError ? (
        <div className="border border-black/15 bg-white/40 p-4 text-sm text-black/70">{emptyMessage}</div>
      ) : listedReviewCount === 0 && !isLoading && loadError ? (
        <div className="border border-black/15 bg-white/40 p-4 text-sm text-black/70">{loadError}</div>
      ) : (
        <div>
          <ReviewList
