...
function ListPreviewStack({ item }) {
  ...
  return (
    <Link
      href={subjectHref || '#'}
      className="group block aspect-2/3 w-full overflow-hidden rounded-[12px] border border-[#0284c7]"
    >
...
function ActivityItem({ isFirst = false, item, variant = 'feed' }) {
  ...
  return (
    <article
      className={
        isShowcase
          ? `border-b border-[#0284c7] ${isFirst ? 'pt-0 pb-4' : 'py-4'} last:border-b-0`
          : `border-b border-[#0284c7] ${isFirst ? 'pt-0 pb-5' : 'py-5'} last:border-b-0`
      }
    >
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
        <div className="min-w-0 text-[1.02rem] leading-7 text-[#0f172a]">
          <span className="font-semibold text-[#0f172a]">{actorName}</span>{' '}
          <span className="text-[#0f172a]">{action}</span>{' '}
          {subjectHref ? (
        ...
        {createdLabel ? (
          <div className="shrink-0 text-sm font-medium text-[#0f172a] sm:pt-0.5">{createdLabel}</div>
        ) : null}
      </div>
    </article>
  );
  ...
  return (
    ...
    >
      {items.length === 0 && isLoading ? (
        <div className="border border-[#0284c7] p-4 text-sm text-black/70">Loading activity...</div>
