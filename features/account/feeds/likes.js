...
const getDefaultFilters = () => ({ media: parseLikesMediaFilters(new URLSearchParams()), listSort: parseListFilters(new URLSearchParams()).sort });

export default function AccountLikesFeed({
  activeSegment, auth, canShowLikesGrid, favoriteShowcase, handleLike,
  handleRequestRemoveLike, handleToggleShowcase, isLikedListsLoading, isOwner,
  isReviewsLoading, isShowcaseSaving, likedLists, likedListsError, likes,
  persistShowcase, reviews, reviewsTotalCount, reviewsError, showcaseMap, watchedItems,
}) {
  ...
  const updateView = (updates) => {
    setViewState((prev) => {
      ...
      if (typeof window !== 'undefined') {
        let qs = buildManagedQueryString(new URLSearchParams(window.location.search), { managedKeys: MEDIA_FILTER_QUERY_KEYS, resetPage: false, values: toMediaQueryValues(next.media) });
        qs = buildManagedQueryString(new URLSearchParams(qs), { managedKeys: LIST_FILTER_QUERY_KEYS, resetPage: false, values: toListQueryValues({ sort: next.listSort }) });
  ...
  const filteredLikes = useMemo(() => applyMediaFilters(likes, viewState.media, { likedKeys }), [likedKeys, likes, viewState.media]);
  ...
  return (
    <>
      {isOwner && activeSegment === 'titles' && (
        <FavoriteShowcaseManager items={favoriteShowcase} isSaving={isShowcaseSaving} onRemoveItem={handleToggleShowcase} onReorder={persistShowcase} />
      )}
        ...
        <AccountMediaGridPage
          ...
          toolbar={
            <AccountMediaFilterBar
              ...
              onReset={(likes.length > 0 && hasMediaFilters) || hasMediaFilters ? () => updateView({ media: getDefaultFilters().media, page: 1 }) : null}
            />
          }
          title="Titles"
        />
        ...
        <AccountPaginatedListGrid
          ...
          toolbar={
            <AccountListSortBar
              ...
              onReset={(likedLists.length > 0 && hasListFilters) || hasListFilters ? () => updateView({ listSort: getDefaultFilters().listSort, page: 1 }) : null}
            />
          }
        />
      )}
  ...
  return (
    <Reorder.Item as="div" value={item} dragListener={false} dragControls={controls} className="relative w-full">
      <div className="flex w-full items-center gap-2 border border-black/15 bg-white/40 px-4 py-3">
        <div onPointerDown={(e) => controls.start(e)} className="center size-8 shrink-0 cursor-grab text-[#475569] transition">
...
function FavoriteShowcaseManager({ items = [], isSaving = false, onRemoveItem, onReorder }) {
  return (
    <AccountSectionLayout icon="solar:star-bold" summaryLabel={`${items.length}/5 selected`} title="Favorites Showcase">
      {items.length === 0 ? <AccountInlineSectionState>No showcase titles selected yet</AccountInlineSectionState> : (
        <Reorder.Group as="div" axis="y" values={items} onReorder={onReorder || (() => {})} className="list-none space-y-2">
          {items.map((item) => (
            <ReorderableListItem
              key={item.id || item.mediaKey || item.entityId}
              item={item}
              renderEditAction={(entry) => (
                <Button variant="destructive-icon" aria-label={`Remove ${entry?.title || entry?.name} from favorites`} disabled={isSaving} onClick={() => onRemoveItem(entry)}>
