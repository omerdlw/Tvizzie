...
function normalizeRatingMode(value, fallback = 'any') {
  const normalized = normalizeString(value).toLowerCase();
  return RATING_MODE_SET.has(normalized) ? normalized : fallback;
}

function normalizeStarValue(value, fallback = 0.5) {
...
function serializeFlagSet(flagSet) {
...
function isSameFilterState(left, right, keys = []) {
