...
import {
  ...
  parseReviewFilters,
  toReviewQueryValues,
} from '@/features/account/filters/reviews/query';
...
export default function AccountReviewsFeed({
  ...
  const likedMediaKeys = useMemo(() => buildReviewMediaKeySet(likes), [likes]);
  const watchedMediaKeys = useMemo(() => buildReviewMediaKeySet(watchedItems), [watchedItems]);
  const rewatchMediaKeys = useMemo(
    () => buildReviewMediaKeySet(watchedItems, (item) => Number(item?.watchCount || 0) > 1),
    [watchedItems]
  );
  ...
  return (
    ...
    >
      ...
      {!enablePagination && hasMore && typeof onLoadMore === 'function' ? (
        <AccountMotionItem className="flex justify-center" index={2}>
          <Button
            type="button"
            onClick={onLoadMore}
