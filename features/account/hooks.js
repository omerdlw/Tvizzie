...
import { filterCollectionItems, showAccountErrorToast } from './account-hook-utils';
...
export function useAccountCollections({
  ...
  const initialLikes = useMemo(
    () => (hasInitialCollectionSnapshot && Array.isArray(initialCollections?.likes) ? initialCollections.likes : []),
    [hasInitialCollectionSnapshot, initialCollections]
  );
  const initialLists = useMemo(
    () => (hasInitialCollectionSnapshot && Array.isArray(initialCollections?.lists) ? initialCollections.lists : []),
    [hasInitialCollectionSnapshot, initialCollections]
  );
  const initialWatchlist = useMemo(
    () =>
      hasInitialCollectionSnapshot && Array.isArray(initialCollections?.watchlist) ? initialCollections.watchlist : [],
    [hasInitialCollectionSnapshot, initialCollections]
  );
  const initialWatched = useMemo(
    () =>
      hasInitialCollectionSnapshot && Array.isArray(initialCollections?.watched) ? initialCollections.watched : [],
    [hasInitialCollectionSnapshot, initialCollections]
  );
  ...
  const [isLoadingCollections, setIsLoadingCollections] = useState(!hasInitialCollectionSnapshot);
  ...
  useEffect(() => {
    ...
    const shouldSubscribeLikes = !shouldScopeCollections || normalizedActiveTab === 'likes';
    const shouldSubscribeLists = !shouldScopeCollections || normalizedActiveTab === 'lists';
    const shouldSubscribeWatched = !shouldScopeCollections || normalizedActiveTab === 'watched';
    const shouldSubscribeWatchlist = !shouldScopeCollections || normalizedActiveTab === 'watchlist';
    ...
    const areAllStreamsResolved = () =>
      resolvedOnce.likes && resolvedOnce.watched && resolvedOnce.watchlist && resolvedOnce.lists;

    setIsLoadingCollections(!hasInitialCollectionSnapshot || !areAllStreamsResolved());
