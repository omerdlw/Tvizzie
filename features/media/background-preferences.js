...
import { notifyPosterPreferenceChange } from '@/core/services/media/user-media';
  ...
  const normalizedEntries = Object.entries(entries)
    ...
    .sort(([, first], [, second]) => {
      const firstUpdatedAt = Math.max(
        Number(first.backgroundUpdatedAt) || 0,
        Number(first.posterUpdatedAt) || 0,
      );
      const secondUpdatedAt = Math.max(
        Number(second.backgroundUpdatedAt) || 0,
        Number(second.posterUpdatedAt) || 0,
      );
...
function setMoviePreferenceByKind(movieId, filePath, kind) {
  ...
  const nextStore = {
    entries: {
      ...(store.entries || {}),
      [resolvedMovieId]: withUpdatedPreferenceEntry(
        store.entries?.[resolvedMovieId],
        kind,
        resolvedFilePath,
      ),
    },
