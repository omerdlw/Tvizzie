...
import { DEFAULT_WATCH_REGION, normalizeWatchRegion, resolveWatchRegionFromBrowser } from '@/core/services/tmdb/watch-region';
import AdaptiveImage from '@/ui/elements/adaptive-image';
const MAX_WATCH_PROVIDERS = 6;
function buildProviderList(watchProviders) {
  const providers = [...(watchProviders?.flatrate || []).map(provider => ({
    ...provider,
    type: 'PLAY'
...
export default function WatchProvidersSurface({
  ...
  useEffect(() => {
    ...
    void requestWatchRegion().then(payload => {
      if (!isActive) {
        return;
      }
  ...
  return <div className="flex w-full flex-col overflow-hidden">
      {providerList.length > 0 ? <div className="flex flex-col px-1">
          {providerList.map(provider => <div key={`${provider.provider_id}-${provider.type}`} className="flex items-center justify-between border-b border-black/10 py-3 last:border-b-0">
              <div className="flex min-w-0 items-center gap-2">
                <AdaptiveImage mode="img" src={`${TMDB_IMG}/w154${provider.logo_path}`} alt={provider.provider_name} loading="lazy" decoding="async" className="h-7 w-7 shrink-0 object-cover" wrapperClassName="h-7 w-7 shrink-0 " />
        ...
        </div> : <div className="center p-4 text-sm">
