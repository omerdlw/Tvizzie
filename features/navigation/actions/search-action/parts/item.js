...
import { applyAvatarFallback, getUserAvatarFallbackUrl, getUserAvatarUrl, resolveImageLoading, resolveImageQuality } from '@/core/utils';
...
export default function SearchResultItem({
  ...
  const rowContent = <div className="flex min-w-0 items-center gap-3">
      <div className={SEARCH_STYLES.thumbnail}>
        {item.media_type === SEARCH_TYPES.USER ? <AdaptiveImage mode="img" className="h-full w-full object-cover" src={userAvatarSrc} alt={title} loading="lazy" decoding="async" fetchPriority="low" onError={event => applyAvatarFallback(event, userAvatarFallbackSrc)} /> : imagePath && !hasImageError ? <AdaptiveImage fill alt={title} className="object-cover" onError={() => onImageError(itemKey)} src={`${TMDB_IMG}/w92${imagePath}`} sizes="64px" loading={resolveImageLoading()} quality={resolveImageQuality('grid')} decoding="async" wrapperClassName="h-full w-full" /> : <div className={`center h-full w-full text-[#7f1d1d]`}>
      ...
      <div className="mr-2.5 flex min-w-0 flex-1 flex-col justify-center gap-1.5">
        ...
        <div className="flex items-center gap-2">
          {year && <div className={SEARCH_STYLES.metaBadge}>
              <span className="px-2 py-1 text-[10px] font-bold tracking-tight text-black/70">{year}</span>
            </div>}
          {director && <div className={SEARCH_STYLES.metaBadge}>
  ...
  return <Link href={detailPath} className={SEARCH_STYLES.resultItem} onClick={event => {
