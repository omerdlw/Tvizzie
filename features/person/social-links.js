...
export default function SocialLinks({
  ...
  return <div className={cn('bg-primary/70 inline-flex h-10 w-fit items-center overflow-hidden border border-black/10 text-black/70 backdrop-blur-md', className)}>
      {availableLinks.map((link, index) => {
      const isLast = index === availableLinks.length - 1;
      return <div key={link.key} className={cn('flex h-10 items-center', !isLast && 'border-r border-black/10')}>
            <Tooltip className="bg-white text-black" text={link.label}>
              <a href={link.getUrl(externalIds[link.key])} target="_blank" rel="noopener noreferrer" title={link.label} className={cn("center hover:text-info h-full shrink-0 p-1 px-2 hover:bg-white", linkClassName)}>
