...
import { MovieSurfaceReveal, getSurfaceItemMotion, getSurfacePanelMotion, useInitialItemRevealEnabled } from '@/features/media/static-route-elements';
...
function normalizeSeasonDetails(seasonDetails = []) {
  return new Map((Array.isArray(seasonDetails) ? seasonDetails : []).filter(season => Number.isFinite(Number(season?.season_number))).map(season => [Number(season.season_number), season]));
}
...
function EpisodeCard({
  episode,
  index = 0,
  shouldAnimateItemReveal = false
}) {
  const title = getEpisodeTitle(episode);
  return <div>
      <MediaCard className="w-[min(18rem,calc(100vw-4.5rem))] sm:w-72" aspectClass="aspect-video" imageSrc={getEpisodeImage(episode)} imageAlt={title} imageSizes="288px" imagePreset="feature" fallbackIcon="solar:panorama-bold" fallbackIconSize={24} topOverlay={<div className="pointer-events-none absolute inset-x-0 bottom-0 bg-linear-to-t from-black/70 via-black/30 to-transparent p-3 text-white">
...
function buildSeasonTabs(seasons, detailsBySeason) {
  return (Array.isArray(seasons) ? seasons : []).filter(season => Number(season?.season_number) > 0).map(season => {
...
export default function TvSeasonsSection({
  ...
  useEffect(() => {
    ...
    setActiveSeasonKey(current => current && seasonTabs.some(season => season.key === current) ? current : seasonTabs[0].key);
  }, [seasonTabs]);
  ...
  return <MovieSurfaceReveal>
      <section className="flex w-full flex-col gap-3">
        <SegmentedControl className="w-full -mx-1" classNames={{
        track: 'px-1',
        wrapper: 'p-0.5',
        button: '',
        indicator: ''
      }} items={seasonTabs} onChange={setActiveSeasonKey} />

        <div className="relative">
          <>
            <div key={`tv-season-${activeSeason.key}`}>
              <Carousel gap="gap-3">
                {episodes.map((episode, index) => <EpisodeCard key={episode.id || `${activeSeason.key}-${episode.episode_number || index}`} episode={episode} index={index} shouldAnimateItemReveal={shouldAnimateItemReveal} />)}
              </Carousel>
            </div>
          </>
        </div>
