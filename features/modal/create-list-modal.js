...
function SearchResultRow({ item, isAdded, onAdd }) {
  ...
  return (
    <button
      ...
      onClick={() => onAdd(item)}
      className={cn(
        'group flex w-full items-center gap-3 rounded-[12px] border px-3 py-2.5 text-left transition-all',
        isAdded
          ? 'cursor-default border-black/10 bg-black/[0.03] opacity-60'
          : 'cursor-pointer border-black/10 hover:border-black/20 hover:bg-black/[0.02]'
      )}
    >
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-black">{title}</p>
        {year !== 'N/A' && (
          <p className="text-[11px] font-medium text-black/50">{year}</p>
        )}
      </div>
      ...
      <span
        className={cn(
          ...
          isAdded
            ...
            : 'border-black/10 text-black/40 group-hover:border-black/30 group-hover:text-black'
        )}
      >
        <Icon icon={isAdded ? 'material-symbols:check-rounded' : 'material-symbols:add-rounded'} size={15} />
      </span>
    </button>
  );
}
...
function DraftItemRow({ index, item, onRemove }) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-[12px] border border-black/5 bg-primary px-3 py-2 transition-all hover:border-black/10">
      <span className="w-5 text-center text-[11px] font-bold tracking-widest text-black/35">{index + 1}</span>
      ...
      <Button
        ...
        className="size-7 shrink-0 opacity-0 rounded-[10px] transition-opacity group-hover:opacity-100 focus-visible:opacity-100"
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="max-h-[42dvh] min-h-0 space-y-1.5 overflow-y-auto overscroll-contain pr-0.5">
          {showSearchResults ? (
            <>
