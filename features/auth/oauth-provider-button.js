...
export const PROVIDER_BUTTON_CLASSNAMES = Object.freeze({
  default:
    'inline-flex w-full items-center gap-3 justify-center rounded-[16px] h-13 px-4 text-black hover:bg-black hover:text-white transition border border-black/10 disabled:cursor-not-allowed disabled:opacity-60',
});
...
export default function OAuthProviderButton({ disabled = false, isBusy = false, mode = 'sign-in', onClick, provider }) {
  ...
  return (
    <Button type="button" onClick={onClick} disabled={disabled} classNames={PROVIDER_BUTTON_CLASSNAMES}>
