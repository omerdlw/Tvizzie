    ...
    <div className="relative" onClick={() => inputRef.current?.focus?.()}>
      <input
        ...
        onPaste={(event) => {
          ...
          const pastedCode = normalizeOtpValue(
        ...
        className="absolute inset-0 z-10 bg-transparent text-transparent outline-none [caret-color:transparent]"
      />
...
export default function AuthVerificationForm({
  ...
  const [expiresAt, setExpiresAt] = useState(
    initialChallenge?.expiresAt || null
  )
  ...
  const [rememberDevice, setRememberDevice] = useState(
    Boolean(data?.rememberDevice)
  )

  const resendRemainingMs = Math.max(
    0,
    resolveVerificationTimestamp(resendAvailableAt) - now
  )
    ...
    async (codeValue = code) => {
      ...
      try {
        ...
        closeVerification(close, {
          ...
          passwordResetProof:
            verificationResult?.passwordResetProof || null,
  ...
  return (
    ...
    >
      ...
      {shouldShowRememberDevice ? (
        <label className="mx-1 flex items-center gap-2 text-[11px] font-semibold tracking-widest text-white uppercase">
          <input
            type="checkbox"
            checked={rememberDevice}
            onChange={(event) => setRememberDevice(event.target.checked)}
            disabled={isSubmitting || isSending}
            className="size-4  border border-white/5  text-white accent-white"
          />
