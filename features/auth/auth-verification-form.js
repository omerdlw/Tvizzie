        ...
        {Array.from({ length: 6 }).map((_, index) => {
          ...
          return (
            <div
              key={`otp-box-${index}`}
              className={cn(
                'center h-13 border border-white/5 bg-transparent text-lg font-semibold',
  ...
  return (
    <form onSubmit={handleSubmit} className={className} aria-busy={isSending || isSubmitting}>
      ...
      <div className="grid gap-2">
        <Button
          className="hover:bg-info h-11 w-full flex-auto border border-white/5 bg-white/5 px-6 text-xs font-bold tracking-widest text-white/70 uppercase hover:text-white"
