    ...
    return (
      <SelectPrimitive.Root value={value} onValueChange={onChange} disabled={disabled}>
        ...
        >
          {leftIcon && (
            <span
              className={cn('inline-flex shrink-0 items-center justify-center', classes.leftIcon)}
            >
              {leftIcon}
            </span>
          )}
          ...
          {rightIcon ? (
            <span
              className={cn('inline-flex shrink-0 items-center justify-center', classes.rightIcon)}
            >
              {rightIcon}
            </span>
          ) : (
            <SelectPrimitive.Icon
              className={cn('inline-flex shrink-0 items-center justify-center', classes.icon)}
            >
              <ChevronDown size={16} />
            </SelectPrimitive.Icon>
          )}
          ...
          >
            <SelectPrimitive.Viewport className={cn(classes.optionsList)}>
              {options.map((option) => (
                ...
                >
                  {option.icon && (
                    <Icon icon={option.icon} size={14} className={cn(classes.optionIcon)} />
                  )}
