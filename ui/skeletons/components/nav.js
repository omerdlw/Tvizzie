...
export function Skeleton({ className }) {
  return (
    <div className={cn('flex h-auto w-full items-center gap-2', className)}>
      <SkeletonBlock className={cn(BLOCK, 'size-12 shrink-0 rounded-[14px]')} />
      <div className="h-full w-full space-y-2">
        <SkeletonBlock className={cn(BLOCK, 'h-4 w-3/4 rounded-full')} />
        <SkeletonBlock className={cn(BLOCK_SOFT, 'h-3 w-1/2 rounded-full')} />
      </div>
    </div>
  );
}
