...
export function SkeletonBlock({ className = '', radius = null, soft = false }) {
  return (
    <div
      className={cn(
        soft ? SKELETON_TOKENS.tone.soft : SKELETON_TOKENS.tone.solid,
...
export function SkeletonLine({ className = '', size = 'md', soft = false }) {
  return (
    <SkeletonBlock soft={soft} radius="full" className={cn(resolveLineHeight(size), className)} />
  );
}
...
export function SkeletonPoster({ className = '', radius = 'card', soft = false }) {
  return (
    <SkeletonBlock soft={soft} radius={radius} className={cn('aspect-2/3 w-full', className)} />
  );
}
