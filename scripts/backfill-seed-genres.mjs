...
async function fetchDiscoverGenreMap(apiKey, targetMediaIds = []) {
  const mediaIdSet = new Set(
    (Array.isArray(targetMediaIds) ? targetMediaIds : []).map((value) => normalizeValue(value)).filter(Boolean)
  );
...
async function main() {
  ...
  const usernames = (Array.isArray(summary?.users) ? summary.users : [])
    .map((user) => normalizeValue(user?.username))
    .filter(Boolean);
  ...
  const allRows = [...likesRowsSource, ...watchlistRowsSource, ...watchedRowsSource, ...listItemRowsSource];
