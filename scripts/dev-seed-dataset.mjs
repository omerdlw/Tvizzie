...
function weightedTimelineDate({ after = null, maxDaysAgo = 420 } = {}) {
  ...
  const lowerBound = after
    ? Math.max(new Date(after).getTime() + randomInt(1, 72) * 60 * 60 * 1000, oldest)
    : oldest;
...
function normalizeTmdbItem(item, entityType) {
  ...
  const title =
    entityType === 'movie' ? item.title || item.original_title : item.name || item.original_name;
...
async function tmdbRequest(path, params = {}) {
  ...
  if (!token) {
    throw new Error(
      'TMDB_API_KEY or NEXT_PUBLIC_TMDB_READ_TOKEN is required unless --skip-tmdb is used.',
    );
  }
...
function pickMedia(pool, { count, movieBias, unique = true }) {
  const selected = [];
  const seen = new Set();
  const movieTarget = Math.min(
    count - 1,
    Math.max(1, Math.round(count * movieBias) + randomInt(-18, 18)),
  );
...
function createMediaPayload(
  item,
  userId,
  { addedAt, position = null, rating = null, updatedAt = addedAt } = {},
) {
  return {
    ...item,
    addedAt,
...
function buildDataset(users, pool) {
  ...
  users.forEach((user, userIndex) => {
    const watchlistItems = pickMedia(pool, {
      count: 400,
      movieBias: user.movieBias + userIndex * 0.015,
    });
    const watchedItems = pickMedia(pool, {
      count: 400,
      movieBias: user.movieBias - 0.08 + userIndex * 0.02,
    });
    const reviewItems = pickMedia(pool, {
      count: 400,
      movieBias: user.movieBias + (user.username === 'dev_nora' ? 0.1 : -0.04),
    });
    const mediaLikeItems = pickMedia(pool, {
      count: user.mediaLikeCount,
      movieBias: user.movieBias,
    });
    ...
    for (let listIndex = 0; listIndex < 30; listIndex += 1) {
      ...
      const items = pickMedia(pool, {
        count: 400,
        movieBias: user.movieBias + randomInt(-20, 20) / 100,
      });
      ...
      items.forEach((item, position) => {
        const addedAt = new Date(
          new Date(createdAt).getTime() + randomInt(0, 90) * 60 * 1000,
        ).toISOString();
        data.listItems.push({
          added_at: addedAt,
          backdrop_path: item.backdrop_path || null,
...
async function runInChunks(label, rows, writer, size = BATCH_SIZE) {
  ...
  for (let index = 0; index < rows.length; index += size) {
    ...
    process.stdout.write(
      `\r${label}: ${Math.min(index + chunk.length, rows.length)}/${rows.length}`,
    );
  }
...
async function clearAllDevelopmentData(admin) {
  ...
  for (const user of authUsers) {
    ...
    if (result.error && result.error.status !== 404) {
      throw new Error(
        `Auth user delete failed for ${user.email || user.id}: ${result.error.message}`,
      );
    }
  }
}
...
async function clearExistingSeedData(admin) {
  ...
  const seedIds = [
    ...new Set([...authUsers.map((user) => user.id), ...profileIds].filter(Boolean)),
  ];
...
async function createSeedUsers(admin) {
  ...
  for (const config of SEED_USERS) {
    ...
    if (profileResult.error) {
      throw new Error(
        `Could not create profile for ${config.email}: ${profileResult.error.message}`,
      );
    }
    ...
    if (usernameResult.error) {
      throw new Error(
        `Could not map username for ${config.email}: ${usernameResult.error.message}`,
      );
    }
...
async function insertDataset(admin, data) {
  ...
  await runInChunks('media reviews', data.mediaReviews, (rows) =>
    admin.from('media_reviews').insert(rows),
  );
  await runInChunks('lists', data.lists, (rows) => admin.from('lists').insert(rows), 200);
  await runInChunks(
    'list items',
    data.listItems,
    (rows) => admin.from('list_items').insert(rows),
    300,
  );
  await runInChunks('list comments', data.listReviews, (rows) =>
    admin.from('list_reviews').insert(rows),
  );
  await runInChunks('review likes', data.reviewLikes, (rows) =>
    admin.from('review_likes').insert(rows),
  );
  await runInChunks('list likes', data.listLikes, (rows) => admin.from('list_likes').insert(rows));
  await runInChunks(
    'activities',
    data.activities,
    (rows) => admin.from('activity').insert(rows),
    300,
  );
}
...
async function main() {
  ...
  if (!options.execute) {
    console.log(
      'Dry run only. Re-run with --yes to clear and recreate the development seed dataset.',
    );
    return;
  }
  ...
  if (pool.movies.length < 400 || pool.tv.length < 400) {
    throw new Error(
      `Media pool is too small. Got ${pool.movies.length} movies and ${pool.tv.length} TV shows.`,
    );
  }
