  ...
  return async function invokeEdgeFunction(
    ...
    try {
      ...
      if (!allowStatuses.includes(response.status)) {
        ...
        const apiMessage = normalizeTrim(safePayload?.error?.message)
...
async function main() {
  ...
  const supabaseUrl = normalizeTrim(
  ...
  try {
    ...
    await logger.runStep('Core edge endpoints are reachable', async () => {
      ...
      assertCondition(
        activityResponse.status !== 404,
        'account-activity-feed must be deployed'
      )
