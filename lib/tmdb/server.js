...
import { isPersonMediaType, normalizeMediaType } from '@/lib/media'
...
function resolveTmdbHeaders() {
  if (!TMDB_API_KEY) {
    throw new Error(
      'TMDB_API_KEY is missing. Configure TMDB_API_KEY or NEXT_PUBLIC_TMDB_API_KEY on the server.'
    )
  }
...
async function tmdbRequest(pathname, { query, revalidate, tags } = {}) {
  const response = await fetch(buildTmdbUrl(pathname, query), {
...
export const getTrending = cache(async (timeWindow = 'day', mediaType = 'movie') => {
  const normalizedMediaType =
    normalizeMediaType(mediaType) === 'movie' ? 'movie' : 'movie'
...
export const discoverContent = cache(
  async ({ genreId, page = 1, sortBy = 'popularity.desc' }) => {
    ...
    const response = await tmdbRequest('/discover/movie', {
      ...
      tags: [
        `tmdb:discover:movie:${normalizedGenre}:${sortBy}`,
...
export const searchContent = cache(
  async (query, searchType = 'movie', page = 1) => {
