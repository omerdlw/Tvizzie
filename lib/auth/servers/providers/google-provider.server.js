import { normalizeProvider } from '@/lib/auth/capabilities'
import {
  GOOGLE_AUTH_INTENTS,
  GOOGLE_AUTH_RESULTS,
  resolveGoogleAuthIntent,
} from '@/lib/auth/servers/providers/google-auth-intent.server'
import {
...
export function isGoogleOAuthSession(decodedToken = {}) {
  ...
  return (
    amrMethods.includes('oauth') && resolveOauthProvider(decodedToken) === GOOGLE_PROVIDER_ID
  )
}
...
export async function unlinkIdentityWithAccessToken({
  ...
  const response = await fetch(
    `${SUPABASE_URL}/auth/v1/user/identities/${normalizedIdentityId}`,
    {
      method: 'DELETE',
      headers: {
...
export async function assertGoogleSessionConsistency({
  ...
  const shouldReject =
    ...
    (result?.profileExists &&
      result?.result === GOOGLE_AUTH_RESULTS.PROVIDER_COLLISION)

  if (!shouldReject) {
    return result
  }

  const googleIdentity = getGoogleIdentity(rawUser)
  let cleanupAttempted = false
  let cleanupSucceeded = false
  let cleanupErrorMessage = null

  if (googleIdentity?.identity_id && accessToken) {
    ...
    try {
      await unlinkIdentityWithAccessToken({
        ...
        fallbackMessage: 'Google provider cleanup failed during session validation',
      })
      cleanupSucceeded = true
    } catch (error) {
      cleanupErrorMessage =
        normalizeValue(error?.message) ||
        'Google provider cleanup failed during session validation'
    }
  }
