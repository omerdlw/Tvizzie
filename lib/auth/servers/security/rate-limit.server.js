...
const MEMORY_STORE_KEY = '__tvizzie_auth_rate_limit_v2_memory_store__'
const RATE_LIMIT_FUNCTION_NAME = 'rate-limit'
const RATE_LIMIT_FALLBACK_FUNCTION_NAME = 'rate-limit-v2'
...
function shouldUseMemoryFallback() {
  const mode = normalizeValue(process.env.RATE_LIMIT_V2_FALLBACK || 'auto')
...
function enforceInMemoryRateLimit({
  ...
  for (const dimension of normalizedDimensions) {
    ...
    if (current > dimension.limit) {
      const retryAfterMs =
        normalizedWindowMs - (now - bucket * normalizedWindowMs)
...
async function enforceEdgeRateLimit({
  ...
  const invokeRateLimitFunction = async (functionName) =>
    fetch(
  ...
  let response = await invokeRateLimitFunction(RATE_LIMIT_FUNCTION_NAME)
  let payload = await response.json().catch(() => ({}))

  if (
    response.status === 404 &&
    RATE_LIMIT_FALLBACK_FUNCTION_NAME !== RATE_LIMIT_FUNCTION_NAME
  ) {
    response = await invokeRateLimitFunction(RATE_LIMIT_FALLBACK_FUNCTION_NAME)
  ...
  if (!response.ok) {
    throw new Error(
      normalizeValue(payload?.error) ||
        `Rate-limit function failed with status ${response.status}`
    )
  }
