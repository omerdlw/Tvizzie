...
export function resolveProviderIds({
  ...
  const providerIdsFromProviderData = Array.isArray(providerData)
    ? providerData
        .map((provider) =>
          normalizeProvider(provider?.providerId || provider?.id)
        )
        .filter(Boolean)
    : []
  ...
  const providerIdsFromTokenMetadata = getMetadataProviders(
...
export function resolveProviderDescriptors({
  ...
  if (Array.isArray(identities)) {
    identities.forEach((identity) => {
      ...
      providerMap.set(providerId, {
        email:
          normalizeEmail(identity?.identity_data?.email || email) || null,
        id: providerId,
        uid:
          normalizeValue(
            identity?.id || identity?.identity_id || identity?.user_id || userId
          ) || null,
      })
    })
  }
...
export function resolveAuthCapabilities({
  ...
  const uniqueProviderIds = uniqueStrings(providerIds)
  ...
  return {
    ...
    googleEnabled,
    primaryProvider,
    needsPasswordSetup: googleEnabled && !passwordEnabled,
